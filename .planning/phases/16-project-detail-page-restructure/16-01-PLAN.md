...
3. **Card 2 - Financial Summary**:
   ...
   - Add refresh button next to Expense label: `<button class="btn btn-sm btn-secondary" onclick="window.refreshExpense()">🔄 Refresh</button>`
   - Remaining Budget: Color-coded (green if positive, red if negative), formula: Budget - Expense
   - Display "—" for expense/remaining if budget not set
...
- Change client_code or project_code to editable fields
  ...
  <verify>
1. `node -e "console.log(require('fs').readFileSync('app/views/project-detail.js', 'utf8').includes('expense-summary-card') && require('fs').readFileSync('app/views/project-detail.js', 'utf8').includes('showExpenseModal'))"`
2. Start dev server and navigate to any project detail page
3. Verify Active toggle appears ABOVE all cards as clickable badge
4. Verify 3 distinct cards are visible with correct field groupings
5. Verify Project Code and Client display as plain text (NOT disabled inputs)
6. Verify Delete button appears BELOW all cards
  </verify>
  <done>
...
- Expense field shows placeholder "—" or amount with "Refresh" button
