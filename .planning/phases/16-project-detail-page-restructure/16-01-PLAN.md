...
<task type="auto">
  <name>Task 1: Restructure layout into 3-card vertical stack with static placeholders</name>
  <files>
    app/views/project-detail.js
  </files>
  <action>
...
3. **Card 2 - Financial Summary**:
   - Fields: Budget (editable), Contract Cost (editable), Expense (STATIC placeholder), Remaining Budget (STATIC placeholder)
   - **CRITICAL - Expense field STATIC only in Task 1:**
     - Display: `<div style="font-weight: 600; color: #64748b; font-size: 1.125rem;">—</div>`
     - NO onclick handler (will be added in Task 2)
     - NO refresh button (will be added in Task 2)
     - Add comment: `<!-- Expense calculation wired in Task 2 -->`
   - **CRITICAL - Remaining Budget STATIC only in Task 1:**
     - Display: `<div style="font-weight: 600; color: #64748b; font-size: 1.125rem;">—</div>`
     - NO calculation logic (will be added in Task 2)
     - Add comment: `<!-- Remaining Budget calculation wired in Task 2 -->`
...
**DO NOT:**
- Add expense calculation logic (Task 2)
- Add onclick handlers for Expense field (Task 2)
- Add refresh button (Task 2)
- Display currentExpense.total (doesn't exist yet - Task 2)
...
- Change client_code or project_code to editable fields
  ...
  <verify>
1. `grep -c "Card 2 - Financial Summary" app/views/project-detail.js` (verify section exists)
2. `grep "Expense calculation wired in Task 2" app/views/project-detail.js` (verify placeholder comment)
3. Start dev server and navigate to any project detail page
4. Verify Active toggle appears ABOVE all cards as clickable badge
5. Verify 3 distinct cards are visible with correct field groupings
6. Verify Project Code and Client display as plain text (NOT disabled inputs)
7. Verify Expense field shows "—" with NO click handler or refresh button
8. Verify Remaining Budget shows "—" (not calculated yet)
9. Verify Delete button appears BELOW all cards
10. Verify no console errors (no references to undefined functions)
  </verify>
  <done>
...
- Expense field shows static placeholder "—" (NO onclick, NO refresh button)
- Remaining Budget shows static placeholder "—" (NO calculation)
- Delete button appears below all cards
- All existing inline editing functionality preserved
- No broken function references (system in working state)
  </done>
</task>

<task type="auto">
  <name>Task 2: Implement expense calculation, update render to wire interactive elements</name>
  <files>
    app/views/project-detail.js
    styles/views.css
  </files>
...
2. **Add required imports at top** (line 6):
```javascript
...
3. **Add expense calculation function** (after `toggleActive()` function):
```javascript
...
4. **Add expense modal function** (after `refreshExpense()`):
```javascript
...
5. **Update init() to calculate expense on load** (after line 132, before renderProjectDetail):
```javascript
// Calculate initial expense
await refreshExpense();
```

6. **Update attachWindowFunctions()** (line 511-515) to include new functions:
```javascript
...
7. **Update destroy()** (line 165-168) to delete new window functions:
```javascript
...
8. **CRITICAL - Update Card 2 rendering in `renderProjectDetail()`** (replace static placeholders from Task 1):

Replace static Expense placeholder with:
```javascript
<!-- Expense field (now interactive) -->
<div style="display: flex; align-items: center; gap: 0.5rem;">
    ...
    <button class="btn btn-sm btn-secondary" onclick="window.refreshExpense()">🔄 Refresh</button>
</div>
...
Replace static Remaining Budget placeholder with:
```javascript
<!-- Remaining Budget (now calculated) -->
${(() => {
...
- Use `getAggregateFromServer()` from finance.js for efficient aggregation
- Modal structure follows existing modal patterns from components.css
- Expense breakdown adapts finance.js expense modal pattern
  </action>
  <verify>
...
5. Verify Expense field now shows calculated amount (not "—")
6. Verify Refresh button appears next to Expense
7. Click Refresh button and verify expense updates with toast
8. Click Expense amount and verify modal opens
9. Verify modal scorecards display: Material Purchases, Transport Fees, Total Expense
10. Verify category breakdown tables render with PO details
11. Close modal with X button and verify modal disappears
12. Verify Remaining Budget calculates and displays with color coding (green if positive, red if negative)
  </verify>
  <done>
- currentExpense state object exists and tracks total, counts
...
- Modal scorecards show Material Purchases, Transport Fees, Total Expense
- Category breakdown tables display with item details (PO ID, item, qty, cost, subtotal)
- Expense field displays total with click handler and refresh button
- Remaining Budget calculates (Budget - Expense) and displays with color coding
...
- init() calls refreshExpense() to calculate on page load
  </done>
</task>
...
3. Verify modal scorecards display: Material Purchases, Transport Fees, Total Expense
4. Verify category breakdown tables render with item details
5. Close modal with X button - verify modal disappears
6. Edit Budget field - verify Remaining Budget recalculates (Budget - Expense)
7. Toggle Active badge (deactivate) - verify confirmation modal appears
8. Toggle Active badge (activate) - verify NO confirmation modal
9. Edit any field and verify inline auto-save still works
10. Verify focus preservation during real-time updates
