...
### Scope resolutions from research (added 2026-08-10, post-RESEARCH.md)

Research (`113-RESEARCH.md`) completed the D-03 audit and surfaced three scope forks. The user resolved all three:

- **D-13:** **Convert the services side in this phase too.** `services.js:893-901`, `mrf-form.js`'s `loadServices()`, and `procurement.js`'s `loadServicesForNewMRF()` still read `assigned_service_codes` (via `getAssignedServiceCodes()`) for the home-department `services_user`. Deleting `syncServicePersonnelToAssignments` per D-08 stops that field being written, so a `services_user` assigned **after** this phase ships would never see their own new service — a services-side variant of the defect this phase exists to fix. D-07's parity mandate and D-08's stop-reading mandate both require this. The phase boundary in `<domain>` is widened accordingly: it is *assignment* read enforcement across `projects` **and** `services`, not `projects` alone.
- **D-14:** **Residual subcollection exposure is DEFERRED, not closed here.** `projects/{id}/activity_entries`, `progress_updates`, `issues`, `audit_log`, `baselines`, `edit_history` all keep `allow read: if isActiveUser()` (`firestore.rules:292,304,312,329,340,353`) and none consult the parent's `personnel_user_ids`. After this phase a scoped user cannot *discover* an unassigned project, but a known or guessed document ID still reads its journal and audit data. The plan MUST state this residual gap explicitly rather than implying read scoping is complete, and it is recorded in `<deferred>` below.
- **D-15:** **Split `projects` into `allow get` / `allow list`, with BOTH scoped on `personnel_user_ids`.** Mirror the structural split that `services` already uses (`firestore.rules:582-603`) but not its hybrid content — do not copy the legacy `isAssignedToService()` branch. A scoped role must not reach an unassigned project by direct doc-ID link either, so `get` is scoped rather than permissive. Per D-02, both branches key on `personnel_user_ids` uniformly; the see-all roles of D-01 (plus the `all_projects` escape hatch of D-09) remain unrestricted on both.
- **D-16:** **Correct D-01's framing.** Research proved `services_admin` is *also* a scoped role for `projects` (`app/utils.js:318` — `PROJECT_SEE_ALL_ROLES` deliberately excludes it), and its mirror `operations_admin` is scoped for `services`. Three MUST-CONVERT sites are only reachable-and-broken because of this. The plan must read D-01 as "`*_user` roles **and** the cross-department admin role are scoped" — a plan written against the literal "only `*_user` roles" wording will miss those sites, exactly as the preliminary audit did.
- **D-17:** **Drop the uncommitted `users.update` carve-out; keep its 8 tests, inverted.** Under D-05/D-08 every client path that wrote `assigned_project_codes` / `assigned_service_codes` cross-department is deleted, so the carve-out grants permissions nothing can exercise — a least-privilege regression with no offsetting benefit. Adapt the 8 working-tree tests to assert those writes now **fail**, preserving them as permanent regression coverage against the rules-layer-drifts-from-the-assignment-model bug class this investigation chain kept rediscovering.
...
- **Scope the `projects` subcollection read rules** (`activity_entries`, `progress_updates`, `issues`, `audit_log`, `baselines`, `edit_history`) to the parent's `personnel_user_ids` — deferred by D-14. Today they are `allow read: if isActiveUser()`, so after Phase 113 a scoped user who knows or guesses a project's document ID can still read its journal and audit trail even though the project itself is no longer discoverable. Perimeter-only scoping; own phase.
