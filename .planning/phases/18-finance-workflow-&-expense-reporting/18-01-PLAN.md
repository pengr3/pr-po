...
<task type="auto">
  ...
  <action>
**Step 0: Add serverTimestamp to imports**
Update line 6 of finance.js to add serverTimestamp to the existing import statement:
...
This is required because approvePRWithSignature() and approveTRWithSignature() use serverTimestamp() for clock-skew protection.

**1. Add global signature pad variable (after line 16):**
Check if `let approvalSignaturePad = null;` already exists at module level (around line 16 after projectExpenses declaration). If it exists, skip this step. If not, add:
```javascript
...
**Why these changes:**
- Step 0 adds serverTimestamp to imports (required for clock-skew protection in approval timestamps)
- Checking for existing approvalSignaturePad variable prevents duplicate declarations
...
- signaturePad.off() prevents memory leaks from event listeners
  ...
  <verify>
1. Read finance.js imports (line 6) → confirm serverTimestamp is included
2. Read finance.js line 16 area → confirm approvalSignaturePad variable declared
3. Start dev server: `python -m http.server 8000`
4. Navigate to Finance > Pending Approvals
5. Click Review on a Material Purchase Request
6. Confirm signature canvas appears with white background and black border
7. Draw signature with mouse → should see smooth black lines
8. Click "Clear Signature" → canvas clears
9. Click "Approve & Generate PO" without signing → should see error toast "Please provide your signature"
10. Draw signature and click Approve → should create PO and close modal
11. Open browser DevTools > Application > IndexedDB > firestore > pos collection
12. Find newly created PO → verify fields exist: finance_signature_url (base64 string starting with "data:image/png;base64,"), finance_approver_user_id, finance_approver_name, date_issued (timestamp)
13. Repeat for Transport Request approval
  </verify>
  <done>
PR/TR approval modals display signature canvas with validation, PO documents contain signature data URL and finance approver attribution, serverTimestamp imported and used correctly
  </done>
</task>
...
9. serverTimestamp is properly imported in finance.js
</verification>
