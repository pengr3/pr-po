...
<task type="auto">
  <name>Task 3: Verify Supplier Management modal consistency</name>
  <files>app/views/procurement.js</files>
  <action>
Verify that Supplier Management tab maintains the correct supplier purchase history modal pattern:

1. Locate the Supplier Management table rendering (in the suppliers section).

2. Verify that supplier names in the table use this pattern:
```javascript
...
3. **Do NOT modify** if already correct. This task is verification only.

4. If the pattern is different or broken, update to match the above pattern.

**Note:** Per Research Pattern #4 (Supplier Name Click Consistency), Supplier Management already has working modal trigger. This task ensures it remains the canonical access point after removing inline links from MRF Records.
  </action>
  <verify>
```bash
# Verify Supplier Management has modal trigger
grep "showSupplierPurchaseHistory" app/views/procurement.js | grep -v "viewPRDetails" | grep -v "viewPODetails"

# Should find the Supplier Management table rendering with onclick handler
grep -B 2 -A 2 "clickable-supplier" app/views/procurement.js
```
  </verify>
  <done>
- Supplier Management table has clickable supplier names
- Clicking supplier name opens purchase history modal
- showSupplierPurchaseHistory() function correctly wired
- This is now the only way to access supplier purchase history
  </done>
</task>
