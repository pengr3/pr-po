...
must_haves:
  truths:
    ...
    - "Timeline captures timestamps (not just dates) for efficiency measurement"
  artifacts:
...
5. Ensure `serverTimestamp` is already imported from firebase.js (should be added in Plan 01).
  </action>
  <verify>
```bash
