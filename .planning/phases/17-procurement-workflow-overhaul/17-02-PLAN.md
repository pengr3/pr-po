...
must_haves:
  ...
  key_links:
    ...
    - from: "viewPRDetails modal"
      to: "PR creator display"
      via: "Modal HTML rendering after table restructure"
      pattern: "viewPRDetails.*pr_creator_name"
---
...
7. **CRITICAL - PR/PO Column Layout:** Render PRs and POs in aligned vertical layout (side-by-side columns). Each cell should display multiple PR or PO IDs as vertical lists, ensuring corresponding PRs and POs are visually aligned in their respective columns. Use consistent spacing and alignment:
```javascript
// PRs column
<td style="padding: 0.75rem 1rem; vertical-align: top;">
    ${prs.map(pr => `<div style="margin-bottom: 0.25rem;">PR-ID</div>`).join('')}
</td>

// POs column (adjacent)
<td style="padding: 0.75rem 1rem; vertical-align: top;">
    ${pos.map(po => `<div style="margin-bottom: 0.25rem;">PO-ID</div>`).join('')}
</td>
```
...
**Why side-by-side PR/PO:** Research Pattern #6 shows users need to see PR-PO relationships at a glance. Aligned columns make this relationship clear.
  </action>
  <verify>
```bash
...
# Verify PR/PO columns use vertical-align: top
grep -A 3 "PRs column\|POs column" app/views/procurement.js | grep "vertical-align: top"
```
  </verify>
  <done>
...
- PRs and POs displayed in aligned side-by-side columns with vertical layout
  </done>
</task>
...
2. **Table Structure:**
   ...
   - PRs and POs displayed side by side in aligned columns
...
5. **Cross-Plan Verification (Plan 17-01 Integration):**
   - Click "View" on a PR in the PRs column
   - PR Details modal should open successfully
   - Modal should display "Prepared By" field with pr_creator_name (from Plan 17-01 Task 3)
   - Verify table restructure did NOT break viewPRDetails() modal functionality
</verification>
...
- [x] PR and corresponding PO displayed side by side in aligned columns
- [x] viewPRDetails() modal still functions correctly after table restructure
</success_criteria>
