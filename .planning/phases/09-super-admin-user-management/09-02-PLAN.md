...
must_haves:
  truths:
    ...
    - "Rejected users are immediately deleted from system"
...
<task type="auto">
  ...
  <files>app/views/user-management.js</files>
  <action>
Add rejection functionality that deletes pending users:
...
1. Create handleRejectUser(userId) function:
   ...
   - On confirm: delete user document
     ```javascript
     await deleteDoc(doc(db, 'users', userId));
     ```
   - Show toast: "User rejected and deleted"
   - Note: Do NOT need to handle Firebase Auth user - they created their own Auth account during registration, but without an active Firestore user doc they cannot use the system
...
3. Create confirmation modal (similar to approval modal structure):
  ...
  <verify>
Click Reject on pending user - confirmation dialog appears. Confirm rejection - user document deleted from Firestore. User disappears from pending list.
  </verify>
  <done>
Rejection functionality deletes pending user registrations immediately with proper confirmation dialog.
  </done>
</task>
...
- Rejected users are deleted immediately (no lingering rejected status)
