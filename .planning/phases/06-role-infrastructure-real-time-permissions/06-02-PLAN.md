...
<task type="auto">
  <name>Task 1: Enhance auth.js with permission integration</name>
...
Login as active user with role -> console shows "[Permissions] Permissions loaded". Logout -> console shows "[Permissions] Destroying permissions observer". No errors in console.
  ...
  <done>
auth.js enhanced: permissions initialize on login for active users with roles, cleanup on logout, role change detection triggers permission reinitialization.
  </done>
</task>
...
**Part C: Listen for permissionsChanged event to update navigation**

Add event listener in auth.js init or at module level:

```javascript
// Update navigation when permissions change (PERM-18)
...
This ensures navigation updates immediately when Super Admin changes role permissions (no logout required).
  </action>
  <verify>
Login as user with role -> navigation shows only permitted tabs. In Firestore, change role permissions -> navigation updates without logout. Finance role user should NOT see Procurement tab, Procurement role user should NOT see Finance tab.
  ...
  <done>
Navigation filtering implemented: nav links have data-route attributes, updateNavForAuth filters based on permissions, permissionsChanged event triggers nav update for real-time permission changes.
  </done>
</task>
...
export async function navigate(path, tab = null, param = null) {
    ...
    if (!publicRoutes.includes(path)) {
        ...
        // Only enforce if permissions are loaded (hasTabAccess exists and returns defined value)
        // If permissions not loaded yet (pending user, no role), allow navigation
        // Full route protection comes in Phase 10
        if (hasAccess === false) {
...
Login as Finance role user -> manually navigate to #/procurement via URL bar -> shows "Access Denied" page. Click "Go to Dashboard" returns to home.
  ...
  <done>
Router enhanced with permission checks: protected routes validate access before navigation, unpermitted access shows Access Denied page with dashboard redirect.
  </done>
</task>
...
- Router blocks access to unpermitted routes with Access Denied page
