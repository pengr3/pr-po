...
<task type="auto">
  ...
  <action>
**Ordering constraint:** There are five logically distinct change locations in this file. They must be applied in this order: (a) module-level `cachedAllMRFs` declaration, (b) cache write in the onSnapshot callback, (c) assignment filter in the onSnapshot callback (using scopedMRFs), (d) `reFilterAndRenderMRFs` function definition, (e) assignmentsChanged listener registration in init() and cleanup in destroy(). The declaration at (a) must exist before any code at (b)-(e) references it. Read all five changes below before touching the file.

---

Two changes to procurement.js. Both are additive. ONLY the MRF list (mrfs tab) is filtered. No other tab is touched.

**Change A: Add module-level cache, assignment filter in onSnapshot, and reFilterAndRenderMRFs function**

Add a module-level variable to hold the unfiltered MRF data. Place it with the other module-level state at the top of the file (around lines 15-29):
...
Insert two blocks between the allMRFs collection loop and the material/transport split. First, cache the raw data. Then, apply the assignment filter. Both go after the `allMRFs.push(...)` loop closes (after line 468) and BEFORE the "Separate by request type" comment (before line 470):
...
Add the reFilterAndRenderMRFs function at module scope (place it near loadMRFs, after the loadMRFs function):
