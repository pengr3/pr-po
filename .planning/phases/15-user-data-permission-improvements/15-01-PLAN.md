...
must_haves:
  truths:
    - "MRF form auto-populates 'Your Name' from logged-in user's full_name"
    - "Requestor name field is not editable by the user"
    - "Submitted MRF document contains the correct requestor_name from auth"
    - "Form reset re-populates the user's name (does not clear it)"
  artifacts:
...
<task type="auto">
  <name>Task 2: Auto-populate requestor name from getCurrentUser() in init()</name>
  <files>app/views/mrf-form.js</files>
  <action>
  ...
  <verify>
  1. Open the app, log in, navigate to MRF form (#/mrf-form). The requestor name field should show your full_name automatically.
  2. Click "Reset Form" and confirm. The field should still show your name after reset.
  3. Submit a valid MRF. After the 2-second reset, the name should still be populated.
  ...
  - Post-submission reset preserves the auto-populated name
  - No console errors
...
4. Reset form -- requestor name is still populated
5. Navigate away and back to MRF form -- requestor name is still populated
</verification>
