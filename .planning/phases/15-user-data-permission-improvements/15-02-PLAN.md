...
<task type="auto">
  ...
  **Modify addProject() function** -- Replace personnel handling:
  ...
  3. In the addDoc call, replace `personnel: personnel || null` with:
     ...
     ```
     Keep the old `personnel` field removal implicit (just don't include it for new projects).

  **Modify saveEdit() function** -- Make personnel optional for editing (backward compat):
  1. Replace the line `const personnel = document.getElementById('personnel').value.trim();` with:
     ...
     if (personnelValue) {
         // If a value is entered, validate it against user list
         const selectedUser = usersData.find(u =>
         ...
         if (selectedUser) {
             ...
             // Allow freetext for backward compatibility with existing projects
             personnelUpdate = { personnel: personnelValue };
         }
     } else {
  ...
  **Modify destroy() function** -- Clear usersData:
  ...
  5. Select a valid user and submit -- project should be created with personnel_user_id and personnel_name fields in Firestore.
  6. Edit an existing project -- personnel field should show the current personnel name, and the datalist should work.
  7. Check Firestore for the new project -- verify personnel_user_id contains a uid and personnel_name contains the full_name.
  </verify>
  <done>
  ...
  - New projects store personnel_user_id and personnel_name (not freetext)
  - Editing existing projects shows current personnel and allows re-selection
  - Backward compatible with existing freetext personnel values
...
<task type="auto">
  ...
  **In init() function** -- Add users listener. After the existing onSnapshot setup for the project, add:
  ...
  ```
  Store the unsubscribe in a variable and clean it up in destroy(). If there is already a listener array or pattern, follow it. Looking at the code, there is a single `listener` variable. Add a new `usersListener` variable at module level:
  ```javascript
  let usersListenerUnsub = null;
  ```
  And assign it: `usersListenerUnsub = onSnapshot(...)`.
  ...
  **In renderProjectDetail()** -- Modify the personnel input (around line ~277).
  ...
  Also, after renderProjectDetail() is called and the DOM is updated, call `populatePersonnelDatalist()`. The easiest way: add `populatePersonnelDatalist();` at the end of the renderProjectDetail() function body.

  **Modify saveField() function** -- Update the personnel case (around line ~327-328):
  Change from:
  ```javascript
  ...
  ```javascript
      ...
      if (trimmedValue) {
          ...
          if (matchedUser) {
              ...
              // Freetext fallback for backward compatibility
              valueToSave = trimmedValue;
          }
      } else {
          valueToSave = null;
          // Also clear personnel_user_id and personnel_name
          try {
              await updateDoc(doc(db, 'projects', currentProject.id), {
                  personnel: null,
  ...
  ```
  NOTE: The saveField function needs to be async (check if it already is). Looking at the code (line ~318), the function uses `await updateDoc`, so it should already be async.
  ...
  **Label update** -- If the label says "Assigned Personnel", keep it as-is. It correctly describes the field's purpose.
  ...
  4. Check Firestore -- the project document should now have personnel_user_id and personnel_name fields.
  5. Refresh the page -- the personnel field should show the selected user's name.
  6. Clear the field and blur -- personnel should be cleared (null in Firestore).
  7. Navigate away from the detail page and check for console errors about leaked listeners.
  </verify>
  <done>
  - Project detail personnel field uses HTML5 datalist with active users
  - Selecting a valid user stores personnel_user_id and personnel_name
  - Clearing the field clears all personnel fields
  - Backward compatible -- existing freetext personnel displays correctly
  - Users listener cleaned up in destroy()
  </done>
</task>
...
5. Select a valid user as personnel and submit -- project created with personnel_user_id in Firestore
6. View the new project on the detail page -- personnel shows the selected user's name
7. Edit personnel on detail page -- datalist works, saving updates personnel_user_id
8. Existing projects with freetext personnel -- still display their old personnel value correctly
</verification>
...
- New projects store personnel_user_id (uid) and personnel_name (denormalized)
- Project detail page personnel inline editing uses the same datalist pattern
- Backward compatibility: existing freetext personnel values display correctly
