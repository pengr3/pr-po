...
must_haves:
  truths:
    ...
    - "Clicking a dropdown item that sets _pendingAdminSection causes admin.js to open the correct section instead of defaulting to users"
  artifacts:
  ...
  key_links:
    ...
    - from: "app/views/admin.js"
      to: "window._pendingAdminSection"
      via: "reads and clears in init() to honor dropdown item selection"
...
- CRITICAL: Before using activeSection, check `window._pendingAdminSection`. If it is set and is a valid section key (exists in SECTIONS), use it as the activeSection override. Then immediately clear it: `delete window._pendingAdminSection`. This is the coordination point with the dropdown in index.html (Plan 02) -- when a user clicks a dropdown item like "Settings", the dropdown sets `_pendingAdminSection = 'settings'` before navigating to #/admin. Without this check, admin.js would always open to the default "users" section regardless of which dropdown item was clicked.
- Set `currentSection = activeSection` (after the _pendingAdminSection override)
...
7. Set `window._pendingAdminSection = 'settings'` in console, then navigate away and back to `#/admin` -- Settings section loads instead of default Users
  </verify>
  <done>Admin wrapper view loads all 3 child views via section switching, properly calling destroy() between switches, reads _pendingAdminSection to honor dropdown item selection, with no modifications to child view modules.</done>
</task>
...
6. Set `window._pendingAdminSection = 'settings'` then navigate to `#/admin` -- Settings section loads
</verification>
...
- init() reads and clears window._pendingAdminSection to honor dropdown section selection
</success_criteria>
