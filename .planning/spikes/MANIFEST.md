...
| 004a | animation-slide | comparison | Given dropdown opens via `.open` class, when CSS transition fires, then content slides down via `translateY` smoothly | PENDING | animation, dropdown, notification |
| 004b | animation-scale | comparison | Given same open trigger, when transition fires, then content scales from top-right origin via `scaleY` | PENDING | animation, dropdown, notification |
| 005 | unread-indicator | standard | Given an unread row, when rendered, then a left-border accent + subtle tint feels distinct without heavy blue fill | PENDING | unread, ux, notification, design |
