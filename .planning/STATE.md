...
> **Prior (planning context):** Phase 104 PLANNED 2026-06-13 — 5 plans / 4 waves. Brings `service-detail.js` to functional parity with the overhauled `project-detail.js` for the 3 subsystems services lack (lifecycle accordion, activity journal, DLP/retention). Copy-then-adapt parity phase (Phase-26 pattern); project-side designs (Phases 100/101/102, all shipped + spike-validated) are the locked baseline — the only intentional divergence is D-13 (recurring On-going stays conservative). Pipeline: gsd-pattern-mapper (`104-PATTERNS.md`, 32KB — verified analog file:line refs + rename map + landmines; CSS confirmed reusable verbatim so NO CSS-foundation plan) → gsd-planner → gsd-plan-checker (2 iterations: 1 BLOCKER + 2 WARNINGS folded in → VERIFICATION PASSED). All CONTEXT.md decisions **D-01..D-15 covered** by ≥1 plan. Commits: `359937c` (initial 5 plans + ROADMAP annotation) + `e77c8a8` (revision — D-12 cost-delta auto-entry added to Plan 02; D-06 proposal-funnel re-verify in Plan 03; explicit enumerated window-fn register↔teardown checks in 02/03/04). **Wave/plan structure:** W1 `104-01` firestore.rules (3 service journal subcollection blocks + audit_log + Finance Record-Release branch under `match /services/{serviceId}` + DEV deploy — `autonomous:false` human gate) → W2 `104-02` (service-detail.js Activity Journal) **‖** `104-05` (procurement.js PO-Delivered service auto-entry join on `service_code` + services.js one-time On-going two-tier signal) → W3 `104-03` (service-detail.js Lifecycle accordion; Completion gate `services_admin`-only per D-04; `computeDlpFields` owned here) → W4 `104-04` (service-detail.js DLP finance bar + tranche editor + `recordServiceRetentionRelease`). The three service-detail.js plans share one file → sequential W2→3→4; disjoint-file work parallelizes in W2. **Landmines pinned in plans:** D-14 `last_activity_at` bump un-awaited/error-swallowed/not-batched; PO-Delivered joins on `service_code` (not project_name); already-ported fns (`computeTrancheLifecycle`/`renderServiceTrancheLifecycle`/`ensureServiceCollectiblesListener`) fenced off; `service-detail.js:7` needs `orderBy`/`limit` imports. Threat models: firestore.rules-focused (T-104-01..18), no HIGH unmitigated; accepted residual T-104-09 (services update rule is role-only/not affectedKeys-masked, so UI Completion-gating is advisory — server-side enforcement deferred, matches D-15). **Carry:** dev rules deploy for UAT + prod `firebase deploy --only firestore:rules` joins the standing v3.3 → main debt (87.4/99/100/101/102/103.1). **Next:** `/clear` → `/gsd-execute-phase 104`.

---

**✅ Phase 103.1 (Priority Feed Signal Accuracy) COMPLETE 2026-06-13 — 4/4 plans / 3 waves, inline sequential on v3.3. VERIFICATION passed; all 3 human gates approved (dev rules deploy + projects UAT [verified against `scripts/seed-dev-projects.js` 4/4/2 dataset] + services UAT).** Makes the Phase 103 Priority Feed trustworthy: replaced the corruptible "days since updated_at" clock with two purpose-built clocks + a two-tier funnel signal matrix. Commits: **01** `d8aa4d1`(status_changed_at at proposals.js::_applyProposalStateTransition [proposal funnel, projects+services] + 4 lifecycle gates + saveField manual status; ISO at funnel/saveField, serverTimestamp at gates)+`ab6540c`(service-detail.js saveServiceField status_changed_at + firestore.rules operations_user allow-list +status_changed_at +last_activity_at, braces 69; services rule unchanged role-only); **02** `4e90171`(last_activity_at fire-and-forget on the 3 journal handlers — postActivityEntry gated on `_addActivityEntry` new success-boolean, submitProgressUpdate/submitNewIssue success-only via throwing await; not awaited/not batched/.catch-swallowed — D-03 landmine); **03** `ef4957e`(projects.js two-tier URGENCY_THRESHOLDS [17 keys] + stageDaysInStage `status_changed_at ?? updated_at ?? created_at` + getProjectSignal rewrite [per-stage watch/urgent FUNNEL map] + On-going on `last_activity_at` two-tier 7/14 + DLP-soon watch 14d + getStageAmbient scoped grey subtext); **04** `af4d503`(services.js mirror + D-05 conservative On-going [watch-only/14d/one-time; recurring suppressed; reads updated_at] + getServiceStageAmbient). SUMMARYs 103.1-01..04. `node --check` PASS all 5 changed JS files; rules braces 69/69; cross-file mirror verified; **NO new portfolio Firestore listener** (onSnapshot 3 each). Operator-locked thresholds (aggressive hot feed): For Proposal 2/5, Internal Approval 2/5, Client Review 7/14, For Revision 2/3, Client Approved 3/7, Mobilization 3/10, Inspection 2/5, On-going 7/14, services On-going watch-cap 14d one-time-only, DLP-soon 14d. **NOTE: several of the plan's literal verify gates had false-positives** (bash `${label}` expansion, over-broad `/var\(--/` matching the `NO var(--*)` comment, a too-tight postActivityEntry window, a `last_activity_at` literal in a services comment [reworded]) — all code confirmed correct via corrected checks; no behavioral impact. **REMAINING (all blocking):** (1) `firebase deploy --only firestore:rules --project dev` [reply "deployed"]; (2) projects browser UAT (SC-1..6 in `103.1-03-PLAN.md`); (3) services browser UAT (SC-3/4/5/6 + D-05 in `103.1-04-PLAN.md`). On all approved → verify_phase_goal → Phase 103.1 COMPLETE. On failure → `/gsd-plan-phase 103.1 --gaps`. Carried prod-rules-deploy debt now includes the 103.1 allow-list (run `firebase deploy --only firestore:rules` at v3.3 → main merge, with 87.4/99/100/101/102).

---

**✅ Phase 103 (Portfolio Table Redesign — Attention Feed + Browse All) COMPLETE 2026-06-13 — 4/4 plans / 4 waves, inline sequential on v3.3. VERIFICATION passed; all 3 browser-UAT gates approved by operator.** One hiccup during UAT: the Priority Feed first rendered unstyled — diagnosed as a stale `views.css` in the browser (hash-SPA never re-fetches CSS on navigation; the file on disk was correct/balanced), resolved by a hard refresh. NOT a code defect, no change made. Spike-033 D+B hybrid: a Priority Feed (urgency-ranked Needs Attention / Worth Watching / On Track) + a Browse All (stage-grouped collapsible) toggle replacing the flat sortable portfolio table in BOTH projects.js and services.js, same URL, persisted in localStorage. Inline per Phase 98/99/100/101/102 precedent (gsd-sdk unavailable + 2 stale locked agent worktrees → worktree/subagent path unsafe). Commits: **01** `bb94453` (shared Phase 103 CSS block in views.css — toggle/feed sections/.feed-row/stage-aware .fin-* finance/.stage-group + W-1 `.feed-row.dlp-*` div-row overrides after `.tier-*`); **02** `3a05b92`(Task 1: URGENCY_THRESHOLDS + normalizeUpdatedAt D-08 dual-shape + getProjectSignal/getDefaultOkSignal + computeUrgencySignals + computeBillingPct→null + renderFinancial D-06 4-state, getDlpState reused)+`fd4374f`(Task 2: vm-toggle + #pdb-feed/#pdb-browse, vmSwitch/renderPortfolio/renderPriorityFeed/buildFeedRow, placeholder renderBrowseAll; W-3 full removal of flat-table+sort+pagination incl. the sortFilteredProjects() call; window.vmSwitch symmetric); **03** `961a36f`(Task 1: STAGE_GROUPS 6 groups/all 10 statuses + getCollapseState/setCollapseState 'browse-collapse' + toggleStageGroup; Completed+Loss default-collapsed)+`6f26ab5`(Task 2: real renderBrowseAll reusing buildFeedRow); **04** `5f1e81b`(Task 1: mirror all helpers into services.js + getDlpState mirrored render-only + SERVICE_STAGE_GROUPS 7 groups incl. leading Draft + 'browse-collapse-services')+`e350fcb`(Task 2: #pdb-feed-svc/#pdb-browse-svc + vmSwitchService/renderServicePortfolio/renderServiceFeed/renderServiceBrowseAll/buildServiceRow/toggleServiceStageGroup; feed/browse render INSIDE the active service_type sub-tab pool D-01; W-3 full removal; both service window fns symmetric). SUMMARYs `103-01..04-SUMMARY.md`. `node --check` PASS both JS files; all CSS markers + cross-view shared-class contract green; **NO new Firestore listener** anywhere (onSnapshot count unchanged 3 each); **NO schema/rules change** (no new fields/collections — does not touch the standing 87.4/99/100/101/102 prod-rules-deploy debt). Scorecards (Phase 92/92.2) + client/search filters + CSV + Phase-7/ASSIGN-04 scope retained above both modes. **⚠ W-2 known partial (CONTEXT-sanctioned, DO NOT re-flag as incomplete D-06):** the On-going "mini utilization bar" ships EMPTY — `collection_tranches` records only {label,percentage,is_retention}, not which tranches are billed, and the portfolio is forbidden from loading billing_requests/collectibles (D-05: no new listener), so computeBillingPct returns null; shows contract value + "{N} tranches defined" only. Full paid-% deferred to a future billing-aware phase. **UAT (all 3 blocking gates APPROVED 2026-06-13):** (1) Plan 02 — projects Priority Feed ✅; (2) Plan 03 — projects Browse All ✅; (3) Plan 04 — services mirror ✅. `103-VERIFICATION.md` status `passed`. **Next (carried, not new to this phase):** v3.3 → main merge + prod `firebase deploy --only firestore:rules` (standing 87.4/99/100/101/102 rules-deploy debt — Phase 103 added NO rules). Potential follow-up phase: billing-aware utilization-% to fill the W-2 empty On-going mini-bar (needs a billing_requests/collectibles listener).
...
Static gates: `node --check` PASS; `buildServicePlanCardHtml` 1 def; `computeServiceProgress` 1 def; `id="servicePlanCard"` 3x (3 return branches); `project-plan-card` CSS class verbatim; Open Plan CTA routes to #/services/${encodeURIComponent(...)}/plan; "No service code" disabled-title; 0 iterStrip/currentIterationLabel; `ensureTasksListener()` called 2x; `currentTasksListenerUnsub` appears 6x (decl+assign+init-teardown x2+destroy-teardown); 0 project_* leaks in new additions.

**PAUSED** at `checkpoint:human-verify` (Task 3) — browser UAT required. See 105-03-SUMMARY.md for 7-step UAT checklist. On "approved" → Phase 105 COMPLETE (3/3 plans).

Carry: prod `firebase deploy --only firestore:rules` rides the standing v3.3 → main merge debt (87.4/99/100/101/102/103.1/104/105-01).
...
- Total plans completed: 234 (v1.0–v3.2)
