...
status: Phase 99.2 (Collectibles Revamp β — Finance Collectibles page) ◆ EXECUTED 2026-06-06 — UAT pending. 3/3 plans / 3 waves, inline sequential on v3.3 (gsd-sdk unavailable + 2 stale locked worktrees → inline per Phase 98/99/99.1 precedent). Commits: 01 `63a27db`+`8a0ebaa` (getCollectibleUrgency/getCollectibleLastPayment single-source helpers D-08a/b + COLLECTIBLES REVAMP β CSS block in views.css); 02 `67604fa` (4-chip reactive scorecard + 10→5 col table w/ 6 urgency borders + Show-N-completed toggle + buildCollectibleCard mobile — all 3 colspan="10"→"5"; CSV/getDisplayedCollectibles untouched D-07); 03 `5bee958`+`8caeab5` (widened billing_requests listener pending+approved split + getOrphanApprovedBillingRequests reconciliation + two-section recovery banner + fileCollectibleFromBanner bridge; approve/reject byte-unchanged D-04). Code-review MD-01 fixed `8207d79` (destroy() now resets collShowCompleted/approvedBillingRequests/approvedBannerCollapsed — restores D-05b hidden-by-default on remount). VERIFICATION `human_needed` (5/5 SCs static PASS; helpers unit-tested); REVIEW clean (0 HIGH, 1 MED fixed, 3 LOW advisory). **7 browser-UAT items in 99.2-HUMAN-UAT.md — phase NOT marked complete until they pass.** Owns the approved-not-filed orphan-state fix deferred from Phase 99 (now implemented). Parent 99-HUMAN-UAT.md untouched (its GAP-1 notifications / GAP-2 tranche-guard are unrelated to 99.2). If any UAT fails → /gsd-plan-phase 99.2 --gaps. ||| Phase 99.1 (Collectibles Revamp α) ✅ COMPLETE 2026-06-06 — 4/4 plans / 3 waves, inline sequential on v3.3. Browser UAT 8/8 PASSED (operator "approved all" — cross-surface service billing flow + project regression); automated verification 8/8 PASS. Verification status → passed; HUMAN-UAT → passed. notifications.js confirmed UNCHANGED (D-26 per-notification link at :448); firestore.rules untouched (D-27). PENDING (carry-overs, not blockers): (a) dev rules deploy `firebase deploy --only firestore:rules --project dev` (Phase 99 rules — no NEW rules this phase); (b) v3.3→main merge + prod `firebase deploy --only firestore:rules` (shared with Phase 87.4/99 deploy debt). ||| Phase 99 CLOSED — UNVALIDATED (2026-06-06): executed 3/3; UAT 5/6 (test-6 notifications PARTIAL). Collectibles spike series 025→027c WRAPPED (spec at .planning/spikes/COLLECTIBLES-REVAMP-SPEC.md).
stopped_at: 2026-06-06 — Phase 99.2 EXECUTED (3/3 plans, inline on v3.3), verification human_needed. Source commits 63a27db/8a0ebaa (01 finance.js helpers + views.css block), 67604fa (02 scorecard+table+cards), 5bee958/8caeab5 (03 banner), 8207d79 (code-review MD-01 destroy() fix); SUMMARYs 1803c96/220ef6a/2047600. 7 browser-UAT items pending in 99.2-HUMAN-UAT.md (surfaces via /gsd-audit-uat). NEXT OPTIONS: (1) browser UAT the 7 items at #/finance → Collectibles (`python -m http.server 8000`) — if all pass, mark 99.2 COMPLETE in ROADMAP/STATE; if any fail → /gsd-plan-phase 99.2 --gaps; (2) deploy dev rules + merge v3.3→main (clears Phase 87.4/99/99.1 deploy debt — NOTE: 99.2 changed NO firestore rules). Phase 99.2 is the last planned phase of the Collectibles Revamp series (025→027c spike series WRAPPED).
