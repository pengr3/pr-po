...
last_updated: "2026-08-13T12:31:43.011Z"
...
| Phase 113 P05 | ~6min | 3 tasks | 3 files |
...
| Phase 114 P04 | 10 min | 2 tasks | 2 files |
...
- [Phase 113-01]: Personnel-derived predicate OR-ed into every isAssignedTo*-gated services/project_tasks/service_tasks branch, legacy term first for short-circuit — D-07/D-08: a user assigned after the legacy sync pipelines retire must keep write authority
- [Phase 113-01]: all_services == true hoisted to a top-level OR term on the services allow list rule — D-09: the escape hatch must be honoured uniformly across every role reaching the branch, not only inside isAssignedToService()
- [Phase 113-01]: No new firestore.rules helper function added; the parent-document personnel check is inlined at all 9 OR-in sites — Plan explicitly forbade adding/removing helper functions; verbosity accepted over an unauthorized new function
...
- [Phase 113-05]: project-plan.js's no-uid scoped project lookup short-circuits to a synthetic empty getDocs result instead of issuing an in:[] query, reusing the existing projSnap.empty render path
- [Phase 113-05]: code-issuance backfill's syncPersonnelToAssignments call deleted outright (not converted) — personnel-authoritative model never needed a backfill
- [Phase 113-05]: expense-modal.js resolves budget+project_code once per invocation via resolvedProjectCode, reused across RFP and collectibles/billing_requests lookups instead of 3 separate fetches
- [Phase 113-06]: Scoped-branch array-contains queries use a single where() clause (no paired equality) to avoid a new composite index; active re-applied client-side. rebuildPSOptions codeless carve-outs kept load-bearing. PO-Delivered legacy-MRF (no project_id) skipped+logged, no name-query fallback. isMrfInAssignedScope left byte-identical (D-04).
- [Phase 113]: proposal-modal.js: resolve the preselected project via a single getDoc() instead of a bare fetch-skip when lockedProjectCode is absent — A naive unconditional skip of the projects fetch would have broken saveProposal()'s project_code resolution for project-detail.js's Start Proposal button (Rule 1 auto-fix)
- [Phase 113]: engagement-create.js: removed the onAfterCreate option from createEngagement's contract entirely, not left as a dead parameter — Grep across app/ confirmed zero remaining consumers after this task's own call site dropped its sync body
...
- [Phase 114-04]: Wired lib/obs.min.js then app/sentry-init.js as classic scripts immediately before </head>, above the module bootstrap; documented version/digest/source URL, SENTRY_RELEASE ritual, and the diagnostics.js client_errors retrofit exemption in CLAUDE.md — no deviations, zero pushes to origin
...
| 260722-msg | Complete 260706-mco's cross-dept mirror on the ONE surface it never touched: the dedicated MRF submission form (app/views/mrf-form.js). Diagnosed via /gsd:debug --diagnose (.planning/debug/crossdept-admin-mrf-filing.md): the picker-visibility gate at ~L406-407 still used the pre-kg0 department-role model — `showProjects` omitted services_admin, `showServices` omitted operations_admin — so `psShowProjects/psShowServices` were false for those admins and rebuildPSOptions() skipped the whole (already assignment-aware) picker block, hiding even ASSIGNED cross-dept projects/services. Contradicted mco's "assigned admin behaves exactly like assigned *_user" principle (the *_user roles were already listed). Fix: 2-line additive — add services_admin to showProjects + operations_admin to showServices. Downstream getAssignedProjectCodes/getAssignedServiceCodes already scope to assigned-only (no leak; home dept see-all via null-return). NO firestore.rules change (mrfs create :399 already permits both admins — block was purely client-side). node --check + greps GREEN. OUT OF SCOPE follow-up: procurement.js:3866 Create-MRF surface is unscoped/over-permissive (shows all projects to everyone) — needs its own reconciliation task. | 2026-07-22 | 1b55dee | SHIPPED TO PROD 2026-07-22 (ff to main 2fe45d1; JS live at clmcop.netlify.app — verified); browser UAT still pending | [260722-msg-let-assigned-cross-dept-admins-file-mrfs](.planning/quick/260722-msg-let-assigned-cross-dept-admins-file-mrfs/) |

## Session Continuity

Last activity: 2026-08-10
Last session: 2026-08-13T12:31:42.981Z
