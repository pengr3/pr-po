...
status: in-progress
stopped_at: Phase 108 EXECUTED (4/4 plans, all waves) + code-verified (108-VERIFICATION.md, 18/18 must-haves, 0 gaps). Phase verification = human_needed — 5 in-browser UAT items pending (108-HUMAN-UAT.md); NOT yet marked phase-complete. Code review DONE (108-REVIEW.md, standard depth, 0 critical / 3 warning / 4 info) — all 3 warnings FIXED (WR-01 collectibles-overdue local-date cutoff not UTC; WR-02 open-issues parent cap split per-collection; WR-03 corrected overstated fail-closed comment), 4 info left advisory. (The phase gsd-verifier + first code-review attempt had earlier died on the account monthly spend limit; both since completed.) Next: run the 5 browser UATs (or /gsd-verify-work 108), then mark 108 complete.
last_updated: "2026-07-11T07:47:58Z"
last_activity: 2026-07-11 — Phase 108 (Home — Per-Role Attention Feeds) EXECUTED via /gsd-execute-phase (4 sequential single-plan waves, gsd-executor per wave, no worktrees). W1 108-01 status-derivation.js (7 pure formulas extract-by-copy; RFP due_date CONFIRMED YYYY-MM-DD). W2 108-02 home-feed-sources.js scaffold + THRESHOLDS + 10 Finance/Procurement sources (4 costly server-side-scoped). W3 108-03 +7 portfolio/admin self-scoped sources (getAssigned*Codes; #6 open-issues bounded fan-out; →17). W4 108-04 moved 3 seeds + ROLE_SOURCES registry + getSourcesForUser + rewired engine (→20 sources; engine pristine; getSourcesForUser re-exported so home.js resolves). Orchestrator independently verified each wave (greps + Python ROLE_SOURCES name-resolution: 20=20, 0 missing; per-role 6/6/6/5/5/6/4). onSnapshot 0/0; node --check clean both files. Verification human_needed (browser UAT only). Prior 2026-07-11: Phase 107 redesign (107.5/107.6) operator-confirmed.
progress:
  total_phases: 7
  completed_phases: 2
  total_plans: 15
...
**Current focus:** v4.2 Polish, Home Revamp & Mobile — Phase 108 (Home — Per-Role Attention Feeds) EXECUTED + code-verified; verification human_needed (5 browser UAT items pending). Next: run browser UAT / /gsd-verify-work 108, then mark 108 complete.

## Current Position

Phase: 108 — Home — Per-Role Attention Feeds — ✅ EXECUTED (4/4 plans, all 4 waves) + code-verified (108-VERIFICATION.md, 18/18 must-haves, 0 gaps) · ⏳ verification human_needed (5 in-browser UAT items, 108-HUMAN-UAT.md). Covers HOME-09…13.  [Prev: 107 ✅ COMPLETE + VERIFIED + REDESIGNED 107.5/107.6, operator-confirmed]
Plan: 4/4 executed (all sequential — shared home-feed-sources.js/home-feed.js surfaces). W1 108-01 ✅ NEW app/status-derivation.js (7 pure formulas extract-by-copy; RFP due_date CONFIRMED YYYY-MM-DD; commits fca5c521/3f5e3be8). W2 108-02 ✅ NEW app/home-feed-sources.js scaffold + THRESHOLDS + 10 Finance/Procurement sources (4 costly server-side-scoped; 737552e6/b33c7d6f/50d87405/159a748c). W3 108-03 ✅ +7 portfolio/admin self-scoped sources (getAssigned*Codes; #6 open-issues bounded chunked fan-out; →17; f47632d9/52273810/0d5a6cc7/1fb92236). W4 108-04 ✅ moved 3 seeds + ROLE_SOURCES registry + getSourcesForUser + rewired engine (→20 sources; engine pristine; getSourcesForUser re-exported for home.js; 616fd985/2b5087c3/7faa35a4).
Status: Phase 108 EXECUTED — orchestrator independently verified every wave. Invariants HELD: Strategy A (onSnapshot 0/0 both files; each source one scoped getDocs; 4 costly sources where()-scoped before client filter); engine pristine (rankItems/dedupeItems/rollUpByCategory/capItems/assembleFeed/SEVERITY + cap 8-25 + roll-up ≥5 unchanged; only import/re-export of getSourcesForUser added); registry integrity (Python name-resolution 20 defined = 20 referenced, 0 missing; per-role 6/6/6/5/5/6/4 exact); self-scoping via getAssigned*Codes ×7/×7; distinct dedupeKey prefixes; case-sensitivity honored; node --check clean both files. Benign engine↔sources SEVERITY cycle (read only in fn bodies) — runtime confirmation is UAT item #1. DEVIATIONS (both benign, executor-flagged): 108-03 reworded 2 comments for grep-invariant hygiene (0 behavior change); 108-04 added `export { getSourcesForUser }` re-export to home-feed.js because home.js:23 imports it by name (prevents module-load break — T-108-05). Locked decisions honored: D-01 bands, D-02 reuse-helpers, D-03 generic ranking+roll-up, D-04 single interleaved list, D-01 services_admin non-approver gate (self-gates to []). Advisory gsd-code-review + phase gsd-verifier both terminated on ACCOUNT MONTHLY SPEND LIMIT (verifier had confirmed "every code-level must-have satisfied" pre-termination; orchestrator wrote 108-VERIFICATION.md inline from its own evidence). NOT yet phase-complete. Next: 5 browser UATs (or /gsd-verify-work 108) → mark complete; /gsd-code-review 108 for deferred review.
Last activity: 2026-07-11 — Phase 108 EXECUTED via /gsd-execute-phase (4 sequential single-plan waves, gsd-executor per wave, no worktrees; orchestrator-owned STATE/ROADMAP writes since gsd-sdk not installed). Prior 2026-07-11: Phase 107 operator-review redesign (107.5/107.6) shipped + operator-confirmed. Carry-overs still open: 5 Phase 108 browser UAT items (108-HUMAN-UAT.md); New Engagement cross-role type-scoping browser check; 107 UAT Tests 4-5 (Phase 108 now supplies the feed rows — re-check together); Phase 106 106-FINDINGS.md AUDIT-02 live data-pass → Phase 112. (Phase 108 code review DONE + 3 warnings fixed 2026-07-12.)
...
4. Mobile optimization — high-traffic views (Home, Projects/Services lists+detail, Clients, MRF Records, Proposals, new modals)
5. Gantt on mobile — view-only (readable/scrollable; editing stays desktop)
6. Data-Layer Audit & Optimization — report→fix pass over the Firestore SDK layer (integrity · correctness · efficiency)

Phase numbering continues from **106** (no reset). Carry-overs NOT in scope: Phase 68.1 (subcon scorecard $0), Phase 70 rework (cancel-PR approval flow), Phase 105.1 (service baseline + plan-iterations parity).
...
| 260615-nzy | Add a "Legacy" group to the Priority Feed + Browse All in both projects and services views. `isLegacyStatus` predicate (non-empty `project_status` not in UNIFIED_STATUS_OPTIONS) pulls legacy rows out of the On Track bucket into a hide-when-empty 🗂️ Legacy Feed section, and adds a predicate-based default-collapsed "Legacy / Unmapped" Browse group (#6b7280). Previously legacy-status engagements were mislabeled under On Track in the Feed and vanished entirely from Browse All. Scorecards untouched (accepted gap). Follow-up 8b132b7: hide the empty Legacy accordion in Browse All. | 2026-06-15 | e80fe99, 59163ef, 8b132b7 | UAT approved 2026-06-15 | [260615-nzy-add-legacy-group-to-priority-feed-and-br](.planning/quick/260615-nzy-add-legacy-group-to-priority-feed-and-br/) |
| 260615-odj | Permission-gated editable project status dropdown in the project DETAIL header (project-detail.js), so legacy/unmapped projects can be re-staged onto a canonical status. Replaces the display-only `hdrStatusBadge` span with a `<select id="hdrStatusSelect">` (UNIFIED_STATUS_OPTIONS) when `showEditControls`; legacy values shown as a selected "(legacy)" option; onchange → existing `saveField('project_status', …)` (stamps status_changed_at, edit history, NOTIF-11). `updateLifecycleBadge` made select-aware (sets `.value`, no longer wipes options via textContent). Fills the gap where the app had NO UI to change a legacy project's status. Follow-up 038cdba: gate the dropdown to LEGACY projects only (canonical projects keep the read-only badge + lifecycle gates). service-detail.js has the same gap (follow-up). | 2026-06-15 | d96650a, 038cdba | UAT approved 2026-06-15 | [260615-odj-editable-project-status-dropdown-on-deta](.planning/quick/260615-odj-editable-project-status-dropdown-on-deta/) |
| 260615-osa | Stage-agnostic "Mark as Loss" action on the project DETAIL lifecycle card (project-detail.js), available at ALL non-terminal stages (excludes Loss/Completed), gated by canDrive. Fills the gap where Loss was only reachable via the proposal flow. Persistent danger button in the lifecycle card chrome + `#projectLossModal` reason modal (min 10 chars). Dual-path `submitProjectLoss`: open linked proposal (status ∉ {client_approved, loss}) → reuses imported `_applyProposalStateTransition` (proposal+project synced, no divergence) + `loss_reason` parity write; else direct project write (`project_status:'Loss'`, `loss_reason`, timestamps) mirroring saveField's edit-history + NOTIF-11. Edits confined to project-detail.js. service-detail.js has the same gap (follow-up). UAT found ops_user permission denied → debug osa-loss-ops-user-permission: loss_reason was missing from the projects ops_user + proposals BRANCH 2 firestore.rules field-masks (494c526, 113e5f6); ALSO discovered MCP firebase_deploy is a no-op — must deploy rules via Firebase CLI. Deployed dev+prod via CLI; UAT approved. | 2026-06-15 | 22fcbdf, 304a264, 494c526, 113e5f6 | UAT approved 2026-06-15 | [260615-osa-stage-agnostic-mark-as-loss-action-on-pr](.planning/quick/260615-osa-stage-agnostic-mark-as-loss-action-on-pr/) |
| 260615-pvd | Port the project-detail.js legacy status dropdown + stage-agnostic Mark-as-Loss to service-detail.js (services parity). FEATURE A: legacy-only `<select id="hdrServiceStatusSelect">` in the service header (reuses saveServiceField('project_status',…) which already stamps status_changed_at + NOTIF-11); badge-updater made select-aware. FEATURE B: gated "Mark as Loss" button in the service lifecycle card chrome + `#serviceLossModal` (min 10 chars) + dual-path `submitServiceLoss` (open parent_collection:'services' proposal → `_applyProposalStateTransition` synced + loss_reason parity; else direct service write + recordEditHistory(...,'services') + NOTIF-11). NO firestore.rules change — services_user update rule is unmasked and proposals BRANCH 2 already permits services + loss_reason. Single file. | 2026-06-15 | b6f4059, 9ba7906 | Awaiting browser UAT | [260615-pvd-port-mark-as-loss-and-legacy-status-drop](.planning/quick/260615-pvd-port-mark-as-loss-and-legacy-status-drop/) |

| 260627-kg0 | Cross-department assignment parity — access becomes assignment-driven (not department-role-locked) for the *_user roles: a services_user assigned to a project (and an operations_user assigned to a service) can see & act on it with full parity. Waves: (1) firestore.rules symmetric assigned-member write gates — project_tasks⇄service_tasks, lifecycle field-masks, audit_log, proposals Branch B; (2) utils.js helpers assignment-driven + procurement.js AND→OR union (preserves 260615-nlj no-leak invariant); (3) UI sweep — detail-page drive gates, proposal-modal canDrive, mrf-form pickers, projects list scope, home dual-department, seed-roles edit flags. UAT follow-ups: persist assigned_*_codes across role changes (reset all_* only); assignment-aware view-only banner; cross-dept assignees stay visible in the Assignments grid. Deps: live role_templates patch (scripts/patch-cross-dept-edit-permissions.js) + PROD rules deploy. | 2026-06-27 | 7ee3c04f, fcbae476, 7ced1f9c, 01785842, 38a3afcb, cabd69ad, 85dfa8fa | SHIPPED TO PROD 2026-06-27 — DEV UAT approved (both directions + 3 follow-ups); merged to main (ff 840fef58); firestore.rules deployed to PROD; live role_templates patched (super_admin); JS live at clmcop.netlify.app | [260627-kg0-cross-dept-assignment-parity](.planning/quick/260627-kg0-cross-dept-assignment-parity/) |
| 260629-jbf | Fix stale-cache bug (root cause: debug session stale-cache-mrf-scoreboard — procurement user served old MRF scoreboard format until hard refresh). De-immutable JS/CSS Cache-Control in BOTH `_headers` and `netlify.toml`: `/*.js` and `/*.css` changed from `public, max-age=31536000, immutable` → `public, max-age=0, must-revalidate` (matches existing `/*` default; both files now byte-identical for JS/CSS). Bare-URL ES modules now revalidate by ETag, so a normal page load AND the existing update banner's `window.location.reload()` pull fresh code — eliminates Layer 1+2 of the diagnosis. Image/font rules left `immutable` (stable, out of scope). Explicitly NOT done: build step, `?v=` import stamping, `app/update-check.js` baseline persistence (Layer 3 — optional follow-up). | 2026-06-29 | 6cd502a, 345bbf8 | Awaiting deploy + PROD response-header check | [260629-jbf-fix-stale-cache-bug-in-both-headers-and-](.planning/quick/260629-jbf-fix-stale-cache-bug-in-both-headers-and-/) |
| 260706-d90 | Rebrand localhost dev banner in `app/firebase.js`: text `DEV ENVIRONMENT — clmc-procurement-dev` → `lintel.build`; colors amber (`background:#f59e0b`/`color:#1e293b`) → slate-dark (`background:#1e293b`/`color:#f8fafc`). Cosmetic-only — `isLocal` guard and all other cssText props (position, z-index, pointer-events, padding, font) unchanged. `node --check` + grep verified. | 2026-07-06 | 883f872 | Awaiting browser UAT | [260706-d90-dev-banner-lintel-build](.planning/quick/260706-d90-dev-banner-lintel-build/) |
| 260706-mco | Cross-department admin scoping — mirror quick 260627-kg0 onto operations_admin/services_admin. An assigned cross-dept admin now behaves like an assigned cross-dept *_user: home dept unchanged (see-all + full admin), cross dept = assignment-scoped visibility + full MEMBER writes (journal/progress/issues, status/lifecycle field-mask, tasks, proposals, audit_log/edit_history); create/delete/personnel stay home-only. 4 layers: (L1) app/utils.js split SCOPE_EXEMPT_ROLES→PROJECT_SEE_ALL_ROLES/SERVICE_SEE_ALL_ROLES via explicit role branches (defuses the Manage-save all_projects:false landmine); (L2) procurement.js isMrfInAssignedScope department-keyed null-handling (preserves 260615-nlj no-leak); (L3) firestore.rules add services_admin to projects assigned-member branches + operations_admin to services branches, symmetric, create/delete byte-unchanged; (L4) project-detail/service-detail canDrive→assignment-gated + assignments grid rows. TDD: scripts/verify-crossdept-admin-scoping.js GREEN (full role×visibility matrix + landmine + codeless/legacy). Plan-checked (1 blocker fixed: test-harness module-const extraction), verified 7/7 must-haves code-static. **PENDING (user):** browser UAT + `firebase deploy --only firestore:rules` to dev then prod (MCP deploy no-ops). | 2026-07-06 | 93734f1, a826cb5, 2ea10ed, 6ac4509, 41c4272 | Verified (code); UAT + rules deploy pending | [260706-mco-cross-department-admin-scoping-mirror-kg](.planning/quick/260706-mco-cross-department-admin-scoping-mirror-kg/) |

## Session Continuity

Last activity: 2026-07-10 - Gathered Phase 107 context (Home — Command Center Shell & Feed Engine). Discussed 4 gray areas → 15 decisions: Command Center = default landing (today's Overview/Engagements/Proposals sub-tabs fold into feed / Your Work / "+ New Proposal" button / non-default Proposals sub-tab); Briefing-stacked single column + condensed nav door rail; flat severity-ranked feed with a 3rd "critical" tier (amends HOME-03), hybrid route-or-modal deep-links, compute-on-load + Refresh; 107 = engine + seed source trio, 108 = per-role definitions; Recent Activity ← notifications, Your Work = 3 hide-empty buckets, role-tailored KPI chips. CONTEXT.md + DISCUSSION-LOG.md committed (9eeced64).
Previous activity: 2026-07-06 - Completed quick task 260706-mco (code): cross-department admin scoping — operations_admin/services_admin now mirror the assigned cross-dept *_user model (home see-all unchanged, cross dept assignment-scoped with full member writes, no cross-dept create/delete). 4 layers (utils.js per-dept split, procurement.js scope-union fix, firestore.rules symmetric member branches, detail/assignments UI). TDD assertion script GREEN; plan-checked + verified 7/7 must-haves. Commits 93734f1/a826cb5/2ea10ed/6ac4509/41c4272 on demo/2026-07-06. PENDING on user: browser UAT + `firebase deploy --only firestore:rules` (dev then prod — MCP deploy no-ops, use CLI). Assign a cross-dept admin via the item's Personnel editor (not the Assignments grid) to bootstrap.
Last session: 2026-07-10
Stopped at: Phase 107 context gathered
Resume file: .planning/phases/107-home-command-center-shell-feed-engine/107-CONTEXT.md
Next action: `/clear` then `/gsd-plan-phase 107` — plan the Home Command Center shell + feed engine (CONTEXT.md committed 9eeced64; 15 decisions across layout/sub-tab fold, feed engine contract, 107↔108 boundary, panel data sources). ⚠ Before/within planning, amend REQUIREMENTS.md HOME-03 (high/medium → critical/high/medium) to match Phase 107 D-06. Prior carry-over loose ends (still open): (1) Phase 86.9 Plan 03 untracked (write SUMMARY-03 + commit/clean DEBUG.md + debug-diag-86.9.js + fix ROADMAP 2/2→3/3); (2) Phase 86.5 (Gantt UI Polish 3) still unplanned; (3) firestore.rules PROD deploy still deferred until v3.3→main merge — NOTE the f785915 project_iterations update rule is now ALSO pending that prod deploy (on top of the Phase 87.4 attachment-gate rule); dev is current, prod is not. Housekeeping: ~17 stale .continue-here files + orphan 83-05 plan + untracked .claude/worktrees/.
