...
**Role templates data (from 06-RESEARCH.md) - ALL 7 TABS FOR ALL 5 ROLES:**
...
**CRITICAL: Each role template MUST include ALL 7 tabs in permissions.tabs object:**
- dashboard, clients, projects, mrf_form, procurement, finance, role_config

For roles without access to a tab, set { access: false, edit: false }.

**Permission structure per role (example for super_admin):**
```javascript
{
  role_id: 'super_admin',
...
**Permission structure for finance role (example of restricted role):**
```javascript
{
  role_id: 'finance',
  role_name: 'Finance',
...
**Add verification function: `verifyRoleTemplates()`**
```javascript
...
<task type="auto">
  <name>Task 3: Seed and verify role templates in Firestore</name>
  <files>N/A - Firestore data operation</files>
  <action>
Run the seeding utility to populate role templates in Firestore, then programmatically verify.

**Steps:**
1. Start local server: `python -m http.server 8000` (if not running)
2. Open browser to http://localhost:8000
3. Open browser DevTools Console (F12)
4. Run seeding with verification chain:
```javascript
import('./app/seed-roles.js').then(async m => {
    await m.seedRoleTemplates();
    const result = await m.verifyRoleTemplates();
    if (result.valid) {
        console.log('[RoleSeeder] SUCCESS: All 5 role templates verified with all 7 tabs');
    } else {
        console.error('[RoleSeeder] FAILED:', result.errors);
    }
});
```
5. Console should show:
   - "[RoleSeeder] Seeding role templates..." (or "already exist")
   - "[RoleSeeder] Verification result: { valid: true, errors: [] }"
   - "[RoleSeeder] SUCCESS: All 5 role templates verified with all 7 tabs"
...
Each document should have: role_id, role_name, permissions.tabs (with all 7 tabs), created_at, updated_at
  </action>
  <verify>
Programmatic verification passes: `verifyRoleTemplates()` returns `{ valid: true, errors: [] }`. Console shows success message. Each role template has all 7 tabs in permissions.tabs.
  </verify>
  <done>
5 role templates seeded in Firestore. Programmatic verification confirms all roles have all 7 tabs (dashboard, clients, projects, mrf_form, procurement, finance, role_config).
  </done>
</task>
...
2. `app/seed-roles.js` exists and exports seedRoleTemplates, forceReseedRoleTemplates, verifyRoleTemplates
...
6. `verifyRoleTemplates()` returns { valid: true, errors: [] }
</verification>

<success_criteria>
- permissions.js module created with all utility functions
- Role template seeding utility created with verification function
- seedRoleTemplates() followed by verifyRoleTemplates() confirms valid structure
- 5 role templates exist in Firestore, each with all 7 tabs
