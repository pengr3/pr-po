...
must_haves:
  truths:
    ...
    - "Expired invitation codes are automatically deleted from database"
  artifacts:
  ...
  key_links:
    ...
    - from: "app/views/user-management.js"
      to: "Firestore invitation_codes"
      via: "deleteDoc for expired codes"
      pattern: "deleteDoc.*invitation_codes"
---
...
1. Module structure:
   - Import db and Firestore functions from firebase.js (including deleteDoc for cleanup)
...
5. init() function:
   ...
   - **IMPORTANT: Call cleanupExpiredCodes() on init to delete expired codes from database**
...
8. renderInvitationCodesTable() function:
   ...
   - **Expiration check logic:** Compare `code.expires_at.toMillis() < Date.now()` to determine if code is expired
   - For unused codes: if expired (expires_at.toMillis() < Date.now()), show "Expired" in red; otherwise show "Expires [relative time]"
   - For used codes: show "Used by [email]" and used_at date
   - Copy button per row for unused, non-expired codes only
   - Mark expired codes visually (lighter text, .expired-code class)

9. cleanupExpiredCodes() function:
   - Query invitation_codes where status === 'active' AND expires_at < Timestamp.now()
   - For each expired code, call deleteDoc to remove from database
   - Log cleanup count to console: "[UserManagement] Cleaned up N expired invitation codes"
   - Run silently (no toast) - this is background maintenance

10. switchUserMgmtTab(tabId) function:
...
11. destroy() function:
  ...
  <verify>
File exists at app/views/user-management.js with render, init, destroy exports. Module loads without errors in browser console. Console shows cleanup log if expired codes exist.
  </verify>
  <done>
user-management.js view module created with invitation code generation, copy-to-clipboard, codes list display, and automatic expired code cleanup functionality.
  </done>
</task>
...
12. **Test expired code cleanup:**
    - In Firestore console, manually create an expired code (set expires_at to past timestamp)
    - Refresh page or re-navigate to User Management
    - Check console for "[UserManagement] Cleaned up N expired invitation codes"
    - Verify expired code is deleted from Firestore
...
- Log in with that email in the app
  ...
  <verify>
Invitation code generation works end-to-end. Generated codes appear in list with correct status and expiration. Expired codes are automatically cleaned up on page load.
  </verify>
  <done>
Invitation code generation and display verified working. Super Admin can generate UUID codes, copy to clipboard, and view all codes with status. Expired code cleanup confirmed working.
  </done>
</task>
...
6. Unused codes show expiration countdown (or "Expired" if past)
...
10. Expired codes are automatically deleted from database on page load
</verification>
...
- Expired codes auto-deleted from database (from CONTEXT.md requirement)
