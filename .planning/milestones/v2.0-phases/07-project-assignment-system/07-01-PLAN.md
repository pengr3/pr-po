...
must_haves:
  truths:
    - "Any view can determine project scope for the current user with a single function call, without duplicating role logic"
    - "An operations_user scoped to specific projects receives exactly that set; an operations_user with the all_projects flag receives no filter; all other roles receive no filter"
    - "A missing or malformed assignment field on an operations_user is treated as zero assignments, not as unconstrained access"
    - "Every view that cares about assignment changes can react to a single event without polling or re-authenticating"
  artifacts:
    - path: "app/utils.js"
