...
5. Confirm the else-branch after the container null check navigates to #/projects
6. Confirm renderProjectDetail() is only called when checkProjectAccess() returns true
  </verify>
  <done>
project-detail.js checks assignment access after loading the project document. If access is denied and the container exists, renders an in-place access denied message with a Back to Projects link. If the container is null (race condition or mid-navigation), falls back to navigating to #/projects. Subscribes to assignmentsChanged to re-check access in real time. Cleanup registered in destroy().
  </done>
</task>
...
5. project-detail.js checkProjectAccess() renders access denied with Back to Projects link when project not assigned and container exists
6. project-detail.js checkProjectAccess() navigates to #/projects when container is null
7. project-detail.js assignmentsChanged listener re-runs checkProjectAccess() when assignments change
8. Both files: no existing functionality removed, all changes are additive insertions
</verification>
...
- Operations User on a removed project's detail page sees access denied with navigation back to project list -- either rendered in place (container exists) or via hash navigation (container null)
