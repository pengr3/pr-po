...
must_haves:
  truths:
    ...
    - "Deactivated users are automatically logged out when status changes (AUTH-09)"
  artifacts:
  ...
  key_links:
    ...
    - from: "app/auth.js"
      to: "users collection"
      via: "real-time listener for status changes"
...
<task type="auto">
  ...
  <action>
    ...
    1. Add initAuthObserver() function:
       ...
       - Module-level variable to track user document listener: let userDocUnsubscribe = null
       - When user signs in:
         ...
         d. Set up real-time listener on user document (onSnapshot) - AUTH-09:
            ```javascript
            userDocUnsubscribe = onSnapshot(doc(db, 'users', user.uid), (docSnapshot) => {
              if (docSnapshot.exists()) {
                const userData = docSnapshot.data();
                currentUser = { uid: user.uid, ...userData };
                // If status changes to 'deactivated', force logout
            ...
            ```
       - When user signs out:
         a. If userDocUnsubscribe exists, call it to clean up listener
         b. Clear currentUser
         c. Dispatch 'authStateChanged' with null
    ...
    The real-time listener ensures AUTH-09: deactivated users are immediately logged out even if already logged in.
  </action>
  <verify>
    Test login flow:
    1. Navigate to #/login
    ...
    8. AUTH-09 test: While logged in, change user status to 'deactivated' in Firestore -> user is immediately logged out
  </verify>
  <done>
...
- Deactivated users are automatically logged out when status changes in Firestore (AUTH-09)
</success_criteria>
