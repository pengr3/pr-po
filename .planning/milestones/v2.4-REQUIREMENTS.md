# Requirements Archive: v2.4 Productivity & Polish
...
- [x] **CLN-01**: Dead files project-assignments.js and service-assignments.js are removed from the codebase
- [x] **CLN-02**: Any other unreferenced/orphaned view or utility files are identified and removed
...
- [x] **MRF-01**: Procurement > MRF Processing "Create MRF" form uses a single unified dropdown listing both projects and services together, replacing the current separated sections
- [x] **MRF-02**: Procurement > Create MRF form pre-fills the Requestor Name field with the logged-in user's full name

### Sortable Tables

- [x] **SORT-01**: Finance user can sort the Material PRs table by clicking column headers
- [x] **SORT-02**: Finance user can sort the Transport Requests table by clicking column headers
- [x] **SORT-03**: Procurement user can sort the MRF Records table by clicking column headers (MRF ID, Date Needed, MRF Status)

### Branding (Phase 47)

- [x] **BRD-02**: Login page displays company logo instead of the "CL" text placeholder

### Data Propagation

- [x] **URG-01**: PR documents carry urgency_level from parent MRF so Finance sees the correct urgency

### Performance Optimization

- [x] **PERF-01**: Firebase offline persistence is enabled so repeat visits load data from IndexedDB cache
- [x] **PERF-02**: All data-loading views show skeleton loading screens (animated placeholders) before real data arrives
- [x] **PERF-03**: Home dashboard uses stale-while-revalidate to show last-known stat values instantly on revisit
- [x] **PERF-04**: Independent data fetches in procurement init() run in parallel (Promise.all)
- [x] **PERF-05**: Reference data (suppliers, projects) uses in-memory cache with TTL to reduce redundant Firestore reads
...
| MRF-01 | Phase 46 | Complete |
| SORT-01 | Phase 47 | Complete |
| SORT-02 | Phase 47 | Complete |
| SORT-03 | Phase 47.1 | Complete |
| BRD-02 | Phase 47 | Complete |
| URG-01 | Phase 47 | Complete |
| MRF-02 | Phase 47.2 | Complete |
...
*Last updated: 2026-03-01 after v2.4 milestone audit*
