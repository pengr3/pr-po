...
- [x] **Phase 87.4: Proposal Modal Polish Pass 2 — Attachment-Required Submission, Action Button Colors, Audit Pill Background Fix, PROP-ID/Version Field Decisions** (INSERTED) — 3/3 plans complete 2026-05-24 (code), UAT + firebase deploy pending in browser. Attachment-gate (3-layer: disabled Submit + hint, handler guard, Firestore rule); Request Revision recolored to `#f59e0b` orange via inline style; dead `<div>v1</div>` Version field removed + grid restructured to 3-column bottom row (Strategy A); root-cause CSS fix in `styles/components.css` (defensive `display:inline-block` + `width:fit-content` on all 9 badge-* classes); PROP-ID stays visible in modal H2 (decision-record only, D-06).
