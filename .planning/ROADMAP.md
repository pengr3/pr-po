...
- [x] **Phase 86.9: Gantt Curtain Divider + PDF Report Export** (INSERTED) � (1) Curtain-style resizable divider: left task-grid is CSS-fixed and the right panel/divider slides over it as a clipping overlay (no column collapse/hide); (2) Export button aligned with Day/Week/Month zoom toolbar � downloads a prototype PDF report containing the Gantt chart snapshot and task completion summary table � completed 2026-05-11 (2/2 plans, UAT approved)
...
- [x] **Phase 91: Navigation Restructuring � MRF into Procurement, My Requests Filtered View, Role Permission Overhaul** � Consolidate navigation by absorbing the standalone Material Request tab into Procurement as a sub-tab (MRFs belong to the procurement flow); retire the redundant My Requests standalone tab and replace it with a 'My Requests' filter toggle inside MRF Records; revise Firestore Security Rules and role configuration to match the new structure
 (completed 2026-05-13)
- [x] **Phase 91.3: RFP Fees — Add Transfer / Cash-Out / Miscellaneous Fees at RFP Creation** (INSERTED) — When creating/processing an RFP, users can add supplementary fees (transfer fees, cash-out fees, and arbitrary miscellaneous fees) that are not covered by the original RFP amount; the RFP total and downstream payables reflect the added fees. The RFP creation flow is overhauled to stay mobile-friendly and ship 100% error-free. (completed 2026-06-04)
- [x] **Phase 91.4: RFP Fee Inclusion in Financial Breakdowns** (completed 2026-06-04) (INSERTED) — RFP fees (transfer_fee/cash_out_fee/misc_fees) are excluded from project/service expense totals across the app. Fix: Tier 1 — expense-modal.js + project-detail.js + service-detail.js (swap amount_requested → getRFPTotal; add rfpFeesTotal accumulation); Tier 2 — finance.js project + service expense dashboards (add post-aggregate RFP fee fetch per project/service). **Plans:** 2
...
- [x] **Phase 97.2: Project Plan Iterations — Save Model Overhaul** (INSERTED) — Rework the iteration save/restore UX: hide auto-snapshots from the rail, add per-card delete buttons, relative timestamps, re-sequenced numbering, and replace window.prompt() with a styled save modal. **Plans:** 3 plans. **(completed 2026-06-03, UAT passed)** — 2 UAT-round fixes shipped in commit f785915: iteration overwrite enabled via field-masked `project_iterations` update rule (deployed to clmc-procurement-dev) and the "Save & Load" infinite-loop fixed (post-save continuation routes to confirmIterLoad, not the openIterConfirm gate). Bug #7 indicator was test-sequencing (no code change).
  Plans:
...
- [x] 86.9-02-PLAN.md -- PDF Export: Export button + exportGanttPDF() + @media print CSS � completed 2026-05-11 (UAT approved)
...
**Plans**: 1 plan
  - [x] 86.7-01-PLAN.md � Phantom drag: _ganttBarDragging flag, mountGanttBarDragGuard(), gated handleGanttDateChange / handleGanttProgressChange, suppressed onSnapshot renderGantt(), destroy() cleanup � COMPLETE (809a416, 8f64888, e557db8)
**UI hint**: yes
...
**Plans**: 5 plans (planned 2026-05-11)
  - [x] 87-01-PLAN.md � Firebase Storage SDK setup + firestore.rules `proposals` block + storage.rules + generateProposalId helper + mount activation (PROP-11) � Wave 1
  - [x] 87-02-PLAN.md � Proposal Dashboard + Create/Edit modal + Detail modal shell with audit trail render (PROP-01, PROP-02, PROP-10, PROP-04 read side) � Wave 2
  - [x] 87-03-PLAN.md � State transitions: Submit/Approve/Reject/Sent/Client Approved/Loss with writeBatch atomicity + NOTIF-09/NOTIF-10 (PROP-03, PROP-04 write side, PROP-07, PROP-11, NOTIF-09, NOTIF-10) � Wave 3
  - [x] 87-04-PLAN.md � Attachment widget: link OR single replaceable file via Firebase Storage (PROP-05, PROP-06 scoped � no version history) � Wave 3
...
- [x] 87.3-03-PLAN.md � project-detail.js + service-detail.js: remove isInProposalRange gate, Start Proposal card path (D-01, D-02, D-05�D-07), renderInlineProposalCard redesign (D-12�D-15), CSS polish � Wave 2
...
- [x] 87.1-05-PLAN.md � project-detail.js + service-detail.js: enriched inline proposal card (attachment, comms, Submit+View buttons); styles/components.css: .proposal-inline-card CSS � Wave 5 � DONE 2026-05-21 (c0fc457 + f6f05b7 + bd02fab)
...
**Wave 1** *(parallel � disjoint files)*
  - [x] 87.2-01-PLAN.md � proposal-modal.js: Target Client auto-select on project change + client.company_name field-name bug fix at 6 sites (1 saveProposal write + 3 dropdown render + 2 sort comparator) (D-01..D-04) � Wave 1
...
### Phase 87.4: Proposal Modal Polish Pass 2 � Attachment-Required Submission, Action Button Colors, Audit Pill Background Fix, PROP-ID/Version Field Decisions (INSERTED)

**Goal:** Close five polish items observed on the proposal modal in a super_admin walkthrough after Phase 87.3 shipped:
  1. **Attachment-required submission** � Submit for Approval must be blocked when no attachment is present (neither link nor file). The modal currently allows attachment-less submissions, which violates the "proposal = the document" intent of Phase 87.
  2. **PROP-YYYY-NNN proposal_id review** � the proposal_id (generated by `app/proposal-id.js`, Phase 87 PROP-01) is rendered in the modal header. Investigation finding: ID is actively used by audit trail, NOTIF-09/NOTIF-10, queue rendering, and home dashboard cards � it is NOT dead. Phase work: confirm with user during discuss-phase whether the visible header surface should remain (recommend: keep � it is the human-readable proposal identifier).
  3. **Request Revision button color** � currently red, conflicts with "Mark as Loss" which already owns the red affordance for terminal failure. Recolor to orange (`#f59e0b`, matching the REVISION_REQUESTED audit dot color set in Phase 87.2-04). Touch only the button in `renderProposalActionButtons`; do not change the audit pill or comms-log styling.
  4. **Version field is dead UI** � `<div>v1</div>` is hardcoded literal in `buildHeader()` at `app/proposal-modal.js:484` with no backing data, no logic, no plan to drive it. Investigation finding: no `version` field exists on `proposals` documents and no caller writes one. Phase work: either remove the field entirely OR scope a real version concept (e.g., bumped on each Revision Requested ? Mark Sent cycle). Default recommendation: **remove** � versioning is not a v4.0 requirement and the hardcoded "v1" misleads users into thinking they can browse history.
  5. **Audit-trail "REVISION REQUESTED" type-pill background** � the type-pill background fills the full row width rather than sizing to its text. Likely a missing `display: inline-block` / `width: fit-content` on the pill class set by Phase 87.2-04 / 87.2-05. Audit ALL audit-action and comms-type pills (`AUDIT_ACTION_LABELS` keys + `COMMS_TYPE_META` classes) to ensure no other type exhibits the same overflow.
**Requirements**: TBD � to be established during /gsd-discuss-phase 87.4
**Depends on:** Phase 87 (proposals collection, proposal_id schema, audit_trail schema, comms_log schema), Phase 87.1 (proposal-modal.js surface), Phase 87.2 (renderProposalActionButtons dual-flag gates, REVISION_REQUESTED audit action, _buildAuditCommsGroups, COMMS_TYPE_META classes � the surfaces being polished), Phase 87.3 (current proposal-modal.js baseline)
**Success Criteria** (what must be TRUE):
  1. Submit for Approval is hard-blocked client-side when `proposal.attachment_kind` is unset (no link, no file) � submit button is either disabled with a helper hint OR submission attempts surface a blocking toast; Firestore rules also reject the write so console bypass is impossible
  2. Request Revision button renders in orange (`#f59e0b`) across all eligible proposal statuses (`pending_client`, plus any other status that surfaces the action); red is now uniquely the Mark as Loss affordance with no other action button sharing it
  3. Hardcoded `<div>v1</div>` Version field in `buildHeader()` is either fully removed (with grid layout adjusted so Target Client / Project span the freed column) OR replaced by a real version value driven by data (user decision recorded in discuss-phase)
  4. PROP-YYYY-NNN proposal_id remains visibly displayed in the modal header (assuming user confirms "keep" during discuss-phase) � no change to the underlying ID generator or schema; phase work is decision-record-only for this item
  5. "REVISION REQUESTED" type-pill in the audit trail and every other audit-action pill + comms-type pill all size to content (not row width); visual scan of a proposal with audit entries spanning CREATED, SUBMITTED, APPROVED, REJECTED, SENT_TO_CLIENT, CLIENT_APPROVED, REVISION_REQUESTED, LOSS plus comms entries of every type shows no full-width pill background
**Plans**: 3 plans
  - [x] 87.4-01-PLAN.md � Attachment-required Submit gate (UI disable + hint + handler guard + Firestore rules) � completed 2026-05-24 (code); UAT + `firebase deploy --only firestore:rules` pending
  - [x] 87.4-02-PLAN.md � Request Revision button recolor (orange #f59e0b) + dead Version field removal (Strategy A grid) � completed 2026-05-24 (code); visual UAT pending
  - [x] 87.4-03-PLAN.md � Audit/comms pill background root-cause CSS fix (code-inspection diagnosis; defensive class-level rule) � completed 2026-05-24 (code); DevTools confirmation + visual UAT pending
**UI hint**: yes
...
**Plans**: 6 plans
Plans:
- [x] 91.3-01-PLAN.md — Lock field strategy + shared legacy-safe fee helpers in utils.js (getRFPTotal/getRFPFees/getRFPFeeBreakdown)
- [x] 91.3-02-PLAN.md — procurement.js: 4-section RFP modal overhaul + progressive-disclosure fee controls + live total + fee write + scoreboard fills
- [x] 91.3-03-PLAN.md — finance.js compute: derivePOSummary/deriveRFPStatus + record-payment math track the fee-inclusive total
- [x] 91.3-04-PLAN.md — components.css: fee-accent chips/rows/cue/breakdown card styles + 44px mobile overrides
- [x] 91.3-05-PLAN.md — finance.js display: incl.-fees cue pill + tooltip/touch-toggle + D-07 breakdown card + PO-summary roll-up
- [x] 91.3-06-PLAN.md — procurement.js: positive-only live validation + disabled-Submit gate + blocked-guard dimming banners
**UI hint**: yes
...
| 91.3 | v4.0 | 6/6 | Complete | 2026-06-04 |
| 91.4 | v4.0 | 2/2 | Complete | 2026-06-04 |
...
Plans:
- [x] 93.1-01-PLAN.md � Queue row-click: add onclick + stopPropagation + hover affordance to _renderHomeApprovalQueueHtml � Wave 1
- [x] 93.1-02-PLAN.md � Unified table + scorecards: replace renderStageGroupCard loop with 5 scorecard tiles + unified table + filter state + window function lifecycle � Wave 2

### Phase 93.2: Proposals Sub-Tab Polish � Active-Only Default Filter, Sort, Pagination, Real-Time Queue, Client Column, Attachment Link (INSERTED)

**Goal:** Six UX improvements to the Home ? Proposals sub-tab: (1) default table filter shows only active-stage proposals (draft, pending_internal, pending_client, for_revision) � closed stages (client_approved, loss) hidden unless their scorecard tile is clicked; (2) proposals table sorted by updated_at descending; (3) proposals table paginated at 10/page using existing `.pagination-*` CSS; (4) approval queue upgraded from one-time getDocs to real-time onSnapshot; (5) Client column (target_client_name) added to queue rows between Submitted By and Amount; (6) clickable attachment paperclip link added to queue rows for direct document access without opening the detail modal.
**Depends on:** Phase 93.1 (home.js proposals sub-tab baseline � unified table + scorecards)
**Requirements**: None mapped (INSERTED UX improvement)
**Plans:** 2/2 plans complete
...
### Phase 96: Proposal Card Redesign — Progress Track + Stat Chips (Spike 009)
...
**Depends on**: Phase 99 (`billing_requests` data + `approveBillingRequest` flow — CLOSED/unvalidated) and existing collectibles infra (`collectibles` collection, `expense-modal.js`). **Now overlaps `finance.js` with Phase 99.2** (this phase makes a minimal approve-bridge/pending-banner generalization for service requests; 99.2 does the full finance revamp on top) — so **99.1 must land before 99.2**. **Must share the money-in vocabulary/formulas** defined in the spec.
**Target files**: `app/views/project-detail.js`, `app/views/service-detail.js`, `app/expense-modal.js`, `app/views/finance.js` (minimal approve-bridge + pending-banner generalization), `app/notifications.js` (service DECIDED route). **No Firestore schema/rules change** (the `billing_requests` rules block is already department-agnostic). **SCOPE EXPANDED 2026-06-06 per user directive:** the *full Phase 99 billing-request flow* is mirrored to services (not just display parity) — services have no billing infra today, so this ports the Initiate Billing modal/submit/listener + generalizes the Finance review side. See `99.1-CONTEXT.md`.
...
**Success Criteria** (what must be TRUE):
  ...
  6. `service-detail.js` reaches **full functional parity** with `project-detail.js`: the 025/026 display revamp AND the ported Phase 99 billing-request flow (Initiate Billing modal → submit → own-requests list). Service `billing_requests` carry `department:'services'`; `finance.js` approve bridge + pending banner are generalized so service requests are reviewable/approvable end-to-end; `BILLING_REQUEST_DECIDED` routes service submitters back to the service detail (CDR-06 / CDR-07 / CDR-08)
**Open questions for discuss/plan**:
...
**Status**: ✅ COMPLETE 2026-06-06 — UAT 7/7 approved (operator "all approved"). Inline sequential on v3.3 (3/3 plans, 3 waves; gsd-sdk unavailable + 2 stale locked worktrees → inline per Phase 98/99/99.1 precedent). Verification `passed` (5/5 success criteria static + 7/7 browser UAT; `99.2-VERIFICATION.md` / `99.2-HUMAN-UAT.md`). Code review **clean** (`99.2-REVIEW.md`): 0 HIGH, 1 MEDIUM fixed inline (`8207d79` — destroy() resets collShowCompleted/approved-banner state, restoring D-05b on remount), 3 LOW advisory. **Owns the approved-not-filed orphan-state fix deferred from Phase 99** — implemented (recovery-by-reconciliation banner).
**⚠ DEFERRED FROM SPIKE 027c (NOT in 99.2 scope — dropped at plan-now scoping, never recorded as deferred; surfaced by operator 2026-06-06 post-UAT):** the 027c filter-bar + sort redesign was in the spike but omitted from `99.2-CONTEXT.md` D-01..D-09. Un-built items (candidate **Phase 99.3 — Collectibles Filter Bar + Urgency Sort**):
  1. Looker Studio-style date-range picker w/ presets (Today / Yesterday / This month / Last 7 / Last 30 / Fixed range / Any time) — replaces the raw `collDueFromFilter`/`collDueToFilter` inputs.
...
**Plans**: 2 plans / 2 waves (planned 2026-06-06; plan-now path — spike-027c + locked decisions are the design contract, no discuss/research, per Phase 99.1/99.2 precedent). finance.js + the COLLECTIBLES REVAMP β CSS block are touched by both plans → SEQUENTIAL (Plan 02 depends_on 01). All UAT browser-gated (zero-build, no test harness; static gate is node --check + grep).
  - [x] 99.3-01-PLAN.md (Wave 1) — Urgency-tier sort (replaces status-priority sort, keys off getCollectibleUrgency) + filter-bar rebuild: free-text search + dept-only dropdown (removes collProjectFilter code-name dropdown) + Looker-style date-range popover (7 presets + Fixed range) + Near-due status option; CSV/pagination/mobile preserved — EXECUTED 2026-06-07 (`b59f77c`, `af2707f`, `cb151dd`; SUMMARY `d585e17`)
  - [x] 99.3-02-PLAN.md (Wave 2, dep 01) — Group-by-Project accordion toggle (flat default ↔ grouped portfolio; groups sorted by worst urgency; flat keeps Show-N-completed + pagination; grouped shows all groups, tranches on expand) — EXECUTED 2026-06-07 (`529ea74`, `e79071d`; SUMMARY `42deaba`)
...
### Phase 102: DLP & Retention Management — Tranche Editor, Completion-Gate DLP Entry, 4-State Display (Spikes 034–036)

**Goal**: Add post-award DLP (Defect Liability Period) & retention tracking to projects — (1) an inline collection-tranche editor in project-detail so On-going projects can add/edit tranches and flag one retention tranche (closes the "tranches locked at creation" gap), (2) DLP field capture (`dlp_months`, `dlp_start_date`, `retention_percentage`) at the existing Gate-4 "Mark Completed" flow, and (3) a 4-state DLP display (On-going → In-DLP amber → Expired red → Released green) across the project-detail finance bar, collection-tranche row tags, info card, and the projects portfolio table — with a Finance-only "Record Release" action — so retention held during DLP is visible end-to-end and never forgotten.
**Depends on**: Phase 99.1 (per-tranche lifecycle footer + `collection_tranches` read display in project-detail), Phase 100 (Gate 4 "Mark Completed" / `lcMarkProjectComplete` — DLP entry extends it), Phase 92 (projects portfolio table — portfolio DLP states attach here). Spikes 034 (hybrid chosen), 035 (VALIDATED), 036 (VALIDATED).
**Target files**: `app/views/project-detail.js` (inline tranche editor + "Ret?" toggle + DLP entry in Gate 4 + `getDlpState()` + 4-state finance bar + retention-tranche row DLP tags + info-card DLP fields + Record Release), `app/views/projects.js` (portfolio-row 4-state DLP visuals), `styles/views.css` (tranche editor + DLP strip + state CSS), `firestore.rules` (project DLP/retention field writes + `retention_released_at` Finance-gated).
**Data model** (new fields on `projects` doc; all read with `|| null` for legacy docs): `dlp_months`, `dlp_start_date`, `dlp_expires_at` (computed), `retention_percentage`, `retention_amount` (computed), `retention_released_at` (null until Finance releases); each `collection_tranches[]` item may carry an `is_retention` boolean (exactly one true).
**Requirements**: Spikes 034 (hybrid) + 035 + 036 verdicts (locked); CONTEXT decisions D-01..D-NN. No new Firestore collection.
**Success Criteria** (what must be TRUE):
  1. **Inline tranche editor** — On-going project-detail shows an "Edit Tranches" control in the Financial card; opens an inline editor (add/remove rows, label + %, live total bar green at exactly 100%); empty state shows a "Set Up Tranches" CTA; Save writes `collection_tranches` back to `projects/{docId}` and validates total = 100% with all labels filled
  2. **Retention flag** — a "Ret?" toggle marks exactly one tranche as the retention tranche (toggling one clears the others); persisted as `is_retention` on that tranche; NO DLP fields captured at editor time (deferred to completion gate per Spike 034 hybrid)
  3. **DLP entry at completion gate** — Gate 4 "Mark Completed" captures `retention_percentage`, `dlp_months`, `dlp_start_date` (defaults to completion date); `dlp_expires_at` + `retention_amount` computed and written to the project doc alongside `project_completed_at`; DLP capture shown/required only when a retention tranche exists
  4. **`getDlpState(project)`** — single source of truth deriving `active | in-dlp | expired | released` from `dlp_months`, `project_status === 'Completed'`, `dlp_expires_at` vs now, and `retention_released_at`; used by every display surface
  5. **4-state finance bar** — project-detail finance bar reflects state: blue utilization (active); amber stacked retention segment + DLP strip "N days remaining" (in-dlp); red strip "Expired N days ago" + Record Release button (expired); green 100% "Fully Collected" (released)
  6. **Tranche row tags + info card** — the retention tranche row shows In DLP / OVERDUE / Released tag matching state; info card surfaces DLP Period + DLP Expires
  7. **Portfolio table states** — `projects.js` portfolio rows show the same 4 states (left-accent border amber/red/green + status tag + bar color)
  8. **Record Release** — Finance-only button (in-dlp + expired states) sets `retention_released_at` via direct write, flips display to green/Released; role-gated in code and `firestore.rules`
  9. **`node --check` PASS** on `project-detail.js` + `projects.js`; all new window functions registered in `init()` and deleted in `destroy()` symmetrically; every new field read with `|| null`; `firestore.rules` updated for the new `projects` fields + `retention_released_at` write gate
**Open questions for discuss/plan**:
