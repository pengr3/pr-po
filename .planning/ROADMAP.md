...
- [ ] **Phase 87.4: Proposal Modal Polish Pass 2 — Attachment-Required Submission, Action Button Colors, Audit Pill Background Fix, PROP-ID/Version Field Decisions** (INSERTED) — Block Submit for Approval when no attachment is present (currently the modal lets a proposal be submitted with neither link nor file); review the PROP-YYYY-NNN proposal_id surfaced in the modal header — confirmed in active use by audit trail / notifications / listings, but user requested explicit confirmation of intent to keep it visible; recolor the Request Revision action button from red to orange (red is semantically reserved for Mark as Loss, the terminal failure action); remove or wire up the hardcoded `<div>v1</div>` Version field in `buildHeader` (currently dead UI with no backing data and no plan to drive it); fix the audit-trail "REVISION REQUESTED" type-pill background filling the full row width instead of sizing to content (audit all comms/audit pill types for the same bug)
...
**Plans**: 3 plans
  - [ ] 87.4-01-PLAN.md — Attachment-required Submit gate (UI disable + hint + handler guard + Firestore rules)
  - [ ] 87.4-02-PLAN.md — Request Revision button recolor (orange #f59e0b) + dead Version field removal
  - [ ] 87.4-03-PLAN.md — Audit/comms pill background root-cause CSS fix (diagnosis-first)
**UI hint**: yes
