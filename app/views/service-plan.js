/* ========================================
   SERVICE PLAN VIEW (Phase 105)
   Full-screen split-pane: hierarchical task list (left) + Frappe Gantt (right).
   Standalone route #/services/:code/plan. Services + projects parity (D-04).
   ======================================== */
...
import { generateServiceTaskId } from '../service-task-id.js';
...
export function render(activeTab = null, param = null) {
    ...
    serviceCode = param;
    if (!serviceCode) {
        return `<div class="empty-state"><h3>Service not specified</h3><p>Use the service list to open a plan.</p></div>`;
    }
    // Initial shell — init() will populate task tree + Gantt after data loads
    return `
        <div class="plan-view-surface" id="planViewSurface">
            <div class="plan-toolbar">
                <a href="#/services/detail/${escapeHTML(serviceCode || '')}" class="plan-back-link">‹ Back</a>
...
export async function init(activeTab = null, param = null) {
    ...
    try {
        ...
        if (projSnap.empty) {
            ...
            if (surface) {
                surface.innerHTML = `<div class="empty-state"><h3>Service not found</h3><p>No service with code <strong>${escapeHTML(serviceCode)}</strong>.</p></div>`;
            }
            return;
        }
        const projDoc = projSnap.docs[0];
        currentService = { id: projDoc.id, ...projDoc.data() };
            ...
            if (tree) {
                tree.innerHTML = `
                    <div class="empty-state">
                        <h3>No service code</h3>
                        <p>This service doesn't have a service code yet. Assign a code to this service, then return to plan tasks.</p>
                    </div>`;
            }
            return;
        }
        ...
        // 4. (Plan 04 will source the assignees picker from `normalizePersonnel(currentService)`
...
async function handleGridCellBlur(taskId, col, rawValue) {
    ...
    try {
        await updateDoc(doc(db, 'service_tasks', taskId), updateData);
        ...
        console.error('[ServicePlan] grid cell write failed:', err);
        showToast(err?.code === 'permission-denied'
            ? `You don't have permission to edit tasks on this service.`
...
async function handleNewRowCommit(input) {
    ...
    const taskId = `TASK-${currentService.service_code}-${maxSeq + 1}`;
    ...
    try {
        await setDoc(doc(db, 'service_tasks', taskId), docData);
        // Snapshot will fire and replace optimistic entry with the server version (idempotent).
    } catch (err) {
        console.error('[ServicePlan] handleNewRowCommit failed:', err);
        ...
        showToast(err?.code === 'permission-denied'
            ? `You don't have permission to add tasks on this service.`
...
async function gridIndentTask(taskId) {
    ...
    try {
        ...
        console.error('[ServicePlan] gridIndentTask failed:', err);
        showToast(err?.code === 'permission-denied'
            ? `You don't have permission to change task hierarchy on this service.`
...
async function gridOutdentTask(taskId) {
    ...
    try {
        ...
        console.error('[ServicePlan] gridOutdentTask failed:', err);
        showToast(err?.code === 'permission-denied'
            ? `You don't have permission to change task hierarchy on this service.`
...
async function gridInsertRowAbove(taskId) {
    ...
    try {
        ...
        for (let attempt = 0; attempt < 5; attempt++) {
            const candidate = await generateServiceTaskId(currentService.service_code);
            const existing = await getDoc(doc(db, 'service_tasks', candidate));
        ...
        for (let i = 0; i < toShift.length; i += CHUNK) {
            ...
            slice.forEach(task => {
                batch.update(doc(db, 'service_tasks', task.task_id), {
        ...
        await setDoc(doc(db, 'service_tasks', newTaskId), {
            task_id: newTaskId,
            service_id: currentService.id,
            service_code: currentService.service_code,
        ...
        console.error('[ServicePlan] gridInsertRowAbove failed:', err);
        showToast(err?.code === 'permission-denied'
            ? `You don't have permission to add tasks on this service.`
...
async function gridPasteRows(afterTaskId) {
    ...
    try {
        ...
        // generateServiceTaskId() reads Firestore each call — calling it N times before any setDoc
        ...
        for (const item of sorted) {
            maxSeq++;
            oldToNew[item._orig_id] = `TASK-${currentService.service_code}-${maxSeq}`;
        }
        ...
        console.error('[ServicePlan] gridPasteRows failed:', err);
        showToast(err?.code === 'permission-denied'
            ? `You don't have permission to add tasks on this service.`
...
async function gridGroupIndent(taskIdsJson) {
    ...
    try {
        for (const id of rootIds) {
            ...
            if (!t) continue;
            const oldParentId = t.parent_task_id || null;
            await updateDoc(doc(db, 'service_tasks', id), { parent_task_id: anchor.task_id, updated_at: serverTimestamp() });
        ...
        console.error('[ServicePlan] gridGroupIndent failed:', err);
...
async function gridGroupOutdent(taskIdsJson) {
    ...
    try {
        for (const id of rootIds) {
            ...
            const oldParentId = t.parent_task_id;
            await updateDoc(doc(db, 'service_tasks', id), { parent_task_id: newParentId, updated_at: serverTimestamp() });
            const i = tasks.findIndex(x => x.task_id === id);
            if (i >= 0) tasks[i] = { ...tasks[i], parent_task_id: newParentId };
            await recomputeParentDates(oldParentId);
            if (newParentId) await recomputeParentDates(newParentId);
        ...
        console.error('[ServicePlan] gridGroupOutdent failed:', err);
...
async function handleRowDrop(draggedTaskId, targetTaskId) {
    ...
    try {
        ...
        console.error('[ServicePlan] handleRowDrop failed:', err);
        showToast(err?.code === 'permission-denied'
            ? `You don't have permission to reorder tasks on this service.`
...
async function handleGroupDrop(selectedIds, targetTaskId) {
    ...
    try {
        ...
        console.error('[ServicePlan] handleGroupDrop failed:', err);
        showToast(err?.code === 'permission-denied'
            ? `You don't have permission to reorder tasks on this service.`
            : 'Could not reorder. Please try again.', 'error');
    }
...
async function commitRowOrderReorder(reorderedTaskIds) {
    ...
    for (let i = 0; i < reorderedTaskIds.length; i += CHUNK) {
        ...
        slice.forEach((taskId, sliceIdx) => {
            ...
            batch.update(doc(db, 'service_tasks', taskId), {
    ...
    _ganttDragMouseupHandler = function(e) {
        ...
        if (_parentDragInfo) {
            ...
            childUpdates.forEach(u => {
                batch.update(doc(db, 'service_tasks', u.taskId), {
            ...
            batch.commit().then(async () => {
                ...
                console.error('[ServicePlan] Parent drag batch write failed:', err);
                showToast(err?.code === 'permission-denied'
                    ? `You don't have permission to edit tasks on this service.`
        ...
        if (_pendingDragWrite) {
            ...
            _pendingDragWrite = null;
            updateDoc(doc(db, 'service_tasks', taskId), {
                start_date: newStart,
                end_date: newEnd,
                updated_at: serverTimestamp()
            }).then(async () => {
                ...
                console.error('[ServicePlan] Phantom drag write failed:', err);
                showToast(err?.code === 'permission-denied'
                    ? `You don't have permission to edit tasks on this service.`
                    : 'Could not save task. Please try again.', 'error');
        ...
        if (_pendingProgressWrite) {
            ...
            updateDoc(doc(db, 'service_tasks', taskId), {
                ...
                console.error('[ServicePlan] Phantom progress write failed:', err);
                showToast(err?.code === 'permission-denied'
                    ? `You don't have permission to edit tasks on this service.`
                    : 'Could not save task. Please try again.', 'error');
                renderGantt(); // revert
...
async function removeArrowDependency(fromId, toId) {
    ...
    try {
        await updateDoc(doc(db, 'service_tasks', toId), {
            dependencies: next,
            updated_at: serverTimestamp()
        });
        // onSnapshot fires renderGantt; arrow disappears naturally.
    } catch (err) {
        console.error('[ServicePlan] removeArrowDependency failed:', err);
        showToast(err?.code === 'permission-denied'
            ? `You don't have permission to edit tasks on this service.`
...
function handleGanttDateChange(task, start, end) {
    ...
    updateDoc(doc(db, 'service_tasks', t.task_id), {
        start_date: newStart,
        end_date: newEnd,
        updated_at: serverTimestamp()
    }).then(async () => {
        ...
        console.error('[ServicePlan] Drag write failed:', err);
        showToast(err?.code === 'permission-denied'
            ? `You don't have permission to edit tasks on this service.`
            : 'Could not save task. Please try again.', 'error');
        renderGantt(); // revert
...
function handleGanttProgressChange(task, progress) {
    ...
    updateDoc(doc(db, 'service_tasks', t.task_id), {
        ...
        console.error('[ServicePlan] Progress write failed:', err);
        showToast(err?.code === 'permission-denied'
            ? `You don't have permission to edit tasks on this service.`
            : 'Could not save task. Please try again.', 'error');
        renderGantt();
...
function computeCriticalPath(taskList) {
    ...
    if (iter >= cap && changed) {
        ...
        console.warn('[ServicePlan] computeCriticalPath: dependency cycle detected, returning empty set');
        return new Set();
    }
...
async function loadBaselines() {
    ...
    try {
        const snap = await getDocs(query(
            collection(db, 'services', currentService.id, 'baselines'),
            orderBy('created_at', 'desc')
        ));
        _baselines = snap.docs.map(d => ({ id: d.id, ...d.data() }));
        const wasCleared = !!localStorage.getItem('baselineCleared_' + currentService.id);
        if (wasCleared) {
            _activeBaselineId = null;
            _baselineData = null;
...
async function saveBaseline(precomputedLabel = null) {
    if (!currentService || tasks.length === 0) {
    ...
    if (precomputedLabel === null) {
        ...
        try {
            const countSnap = await getDocs(collection(db, 'services', currentService.id, 'baselines'));
    ...
    try {
        ...
        await addDoc(collection(db, 'services', currentService.id, 'baselines'), {
            label,
            created_at: serverTimestamp(),
            tasks: tasksMap
        });
        ...
        if (currentService?.id) localStorage.removeItem('baselineCleared_' + currentService.id);
        await loadBaselines();
        injectBaselineOverlay();
        renderSlipSummary();
        updateBaselineToolbarUI();
...
function clearBaseline() {
    _activeBaselineId = null;
    if (currentService?.id) localStorage.setItem('baselineCleared_' + currentService.id, '1');
    _baselineData = null;
    const ganttSvg = document.querySelector('#ganttPane svg');
    if (ganttSvg) {
        ganttSvg.querySelectorAll('.gantt-baseline-outline, .gantt-slip-badge, .gantt-slip-badge-bg').forEach(el => el.remove());
...
async function deleteBaseline(id) {
    ...
    modal.querySelector('#blDeleteConfirm').addEventListener('click', async () => {
        modal.remove();
        try {
            await deleteDoc(doc(db, 'projects', currentService.id, 'baselines', id));
...
// Reads last_loaded_iteration_id from currentService and sets _loadedIterationId / _loadedIterationLabel
...
function applyLastLoadedFromProject() {
    ...
    const storedId = currentService.last_loaded_iteration_id;
    if (!storedId) return;
    ...
    if (!iter || iter.auto) {
        ...
        updateDoc(doc(db, 'projects', currentService.id), { last_loaded_iteration_id: null })
            .catch(() => {});
        return;
    }
...
async function loadIterations() {
    if (!currentService) { _iterations = []; return; }
    try {
...
async function restoreIteration(iterationId) {
    ...
    try {
        ...
        const allOps = [
            ...
            ...toWrite.map(t  => ({ type: 'set', ref: doc(db, 'service_tasks', t.id), data: (({ id, ...rest }) => rest)(t) })),
        ];
        ...
        try {
            await updateDoc(doc(db, 'projects', currentService.id), {
...
async function undoIterRestore() {
    ...
    try {
        ...
        const allOps = [
            ...
            ...autoSnap.tasks.map(t => ({ type: 'set', ref: doc(db, 'service_tasks', t.id), data: (({ id, ...rest }) => rest)(t) })),
        ];
        for (let i = 0; i < allOps.length; i += OPS_PER_BATCH) {
            const chunk = allOps.slice(i, i + OPS_PER_BATCH);
            const batch = writeBatch(db);
...
function renderCustomGanttHeader() {
    ...
    try {
        ...
        console.warn('[ServicePlan] Custom header render failed:', e);
    }
}
    ...
    if (svg.dataset.linkBound !== '1') {
        ...
        svg.addEventListener('mouseup', async (e) => {
            ...
            if (!toWrapper || toWrapper.dataset.id === fromTaskId) {
                ...
                try {
                    await updateDoc(doc(db, 'service_tasks', fromTaskId), { end_date: newEndDate, updated_at: serverTimestamp() });
                    ...
                    console.error('[ServicePlan] bar resize failed:', err);
            ...
            try {
                ...
                await updateDoc(doc(db, 'service_tasks', toTaskId), {
                ...
                console.error('[ServicePlan] drag-to-link write failed:', err);
...
async function deleteTaskNow(taskId) {
    ...
    try {
        ...
        for (let i = 0; i < allIds.length; i += CHUNK) {
            ...
            const batch = writeBatch(db);
            slice.forEach(id => batch.delete(doc(db, 'service_tasks', id)));
            await batch.commit();
        }
        ...
        console.error('[ServicePlan] deleteTaskNow failed:', err);
        if (err?.code === 'permission-denied') {
            showToast(`You don't have permission to edit tasks on this service.`, 'error');
        } else {
...
async function recomputeParentDates(parentTaskId, excludeIds = null) {
    ...
    try {
        await updateDoc(doc(db, 'service_tasks', parentTaskId), {
            start_date: minStart,
            end_date: maxEnd,
            updated_at: serverTimestamp()
        });
        ...
        console.warn('[ServicePlan] recomputeParentDates: write failed for parent', parentTaskId, err?.code);
        if (err?.code === 'permission-denied') {
...
async function applyFsAutoSchedule(successorId, predecessorId) {
    ...
    try {
        await updateDoc(doc(db, 'service_tasks', successorId), {
            start_date: newStart,
            end_date: newEnd,
            updated_at: serverTimestamp()
        });
        ...
        console.warn('[ServicePlan] FS auto-schedule shift failed:', err);
    }
}
