...
export async function init(activeTab = 'form') {
    ...
    try {
        ...
        // loadProjects/loadServices, which filter by assigned codes).
        // Quick 260706-mco / 260722-msg: department admins are also cross-dept assignment-scoped here —
        // services_admin/operations_admin now get both pickers too; do not re-introduce the single-picker
        // omission (see rebuildPSOptions()'s getAssignedProjectCodes/getAssignedServiceCodes gating).
        const showProjects = ['super_admin', 'finance', 'procurement', 'operations_admin', 'services_admin', 'operations_user', 'services_user'].includes(role);
        const showServices = ['super_admin', 'finance', 'procurement', 'services_admin', 'operations_admin', 'services_user', 'operations_user'].includes(role);
